...
export default function Home() {
  ...
  const [tab, setTab] = useState<"board" | "games" | "rules" | "reports" | "settings">("board");
  ...
  return <main>
    ...
    {tab === "reports" && <Reports rows={report} squares={squares} visitorDigits={visitorDigits} homeDigits={homeDigits} me={me!} season={season} generatingFlyer={generatingFlyer} onGenerateFlyer={openReportFlyer} />}
    {tab === "settings" && me?.role === "admin" && <Settings season={season} seasonArchives={seasonArchives} members={members} activity={activity} me={me} saving={saving} onSaveMember={saveMember} onRemoveMember={removeMember} onSaveSeason={saveSeason} />}
...
function Reports({ rows, squares, visitorDigits, homeDigits, me, season, generatingFlyer, onGenerateFlyer }:{ rows:ReportRow[]; squares:Square[]; visitorDigits:string; homeDigits:string; me:Member; season:SeasonConfig; generatingFlyer:boolean; onGenerateFlyer:()=>void }) {
  const [reportView,setReportView] = useState<"summary"|"squares">("summary");
  ...
  return <section className="content reports-section"><div className="section-heading"><div><p className="kicker">Control y seguimiento</p><h3>Reportes</h3></div><span className="year-pill">{squares.filter((s)=>s.status!=="available").length} vendidas</span></div>
    <div className="report-switch"><button className={reportView==="summary"?"active":""} onClick={()=>setReportView("summary")}>Sumario por socio</button><button className={reportView==="squares"?"active":""} onClick={()=>setReportView("squares")}>Detalle de casillas</button></div>
