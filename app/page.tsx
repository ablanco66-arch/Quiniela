...
async function createWinnerFlyer({result,game,square,visitorDigit,homeDigit,seasonName,gamePrize}:WinnerRow){
  ...
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 28px ${sportFont}`;ctx.fillText(`QUINIELA MNF ${seasonName}`,540,276);ctx.fillStyle=white;canvasFontToFit(ctx,"¡TENEMOS GANADOR!",940,78,sportFont,48);ctx.strokeStyle=navy;ctx.lineWidth=9;ctx.strokeText("¡TENEMOS GANADOR!",540,363);ctx.fillText("¡TENEMOS GANADOR!",540,363);ctx.fillStyle=cream;canvasFontToFit(ctx,"TU PARTICIPACIÓN HACE POSIBLE GRANDES CAUSAS",900,25,sportFont,18);ctx.fillText("TU PARTICIPACIÓN HACE POSIBLE GRANDES CAUSAS",540,410);
  ...
  roundedBox(ctx,350,438,380,54,27,gold,null,0);ctx.fillStyle=navy;ctx.font=`400 23px ${sportFont}`;ctx.fillText(`${game.date} ${seasonName}`.toUpperCase(),540,474);
  ...
  ctx.save();ctx.shadowColor="#f7b50066";ctx.shadowBlur=36;ctx.fillStyle=gold;ctx.beginPath();ctx.arc(540,940,119,0,Math.PI*2);ctx.fill();ctx.restore();ctx.fillStyle=navy;ctx.font=`400 27px ${sportFont}`;ctx.fillText("PREMIO",540,907);ctx.font=`400 67px ${sportFont}`;ctx.fillText(`$${gamePrize}`,540,971);ctx.font=`400 23px ${sportFont}`;ctx.fillText("USD",540,1006);
...
async function createSquareDetailPdf(squares:Square[],visitorDigits:string,homeDigits:string,season:SeasonConfig,squareFilter:string) {
  ...
  const left=11,tableWidth=188,rowHeight=8.2,headerY=65,footerY=274;
  ...
  const drawHeader=()=>{doc.setFillColor(navy);doc.rect(0,0,210,51,"F");doc.setFillColor(gold);doc.rect(0,0,210,2.2,"F");doc.addImage(clubLogo,"PNG",10,8,48,21.5);doc.addImage(mnfLogo,"PNG",179,7,22,22);doc.setTextColor(gold);doc.setFont("helvetica","bold");doc.setFontSize(13);doc.text(`QUINIELA MONDAY NIGHT FOOTBALL ${season.name}`,117,16,{align:"center"});doc.setTextColor(white);doc.setFontSize(17);doc.text("DETALLE DE CASILLAS",117,25,{align:"center"});doc.setFont("helvetica","normal");doc.setFontSize(8);doc.setTextColor("#c5d0df");doc.text(`Generado el ${generatedDate}`,117,33,{align:"center"});doc.setDrawColor("#ffffff");doc.setLineWidth(.25);for(let x=65;x<172;x+=12){doc.line(x,40,x+5,40);}doc.setTextColor(navy);doc.setFont("helvetica","bold");doc.setFontSize(9);doc.text(`${records.length} CASILLAS${squareFilter!=="all"?` · ${squareFilter.toUpperCase()}`:""}`,left,59);};
  const drawTableHeader=()=>{doc.setFillColor(gold);doc.rect(left,headerY,tableWidth,9,"F");doc.setTextColor(navy);doc.setFont("helvetica","bold");doc.setFontSize(7);columns.forEach((column)=>doc.text(column.label,column.align==="center"?column.x+column.w/2:column.x+2,headerY+5.8,{align:column.align}));};
  const drawFooter=(page:number,total:number)=>{doc.setFillColor(navy);doc.rect(0,footerY,210,23,"F");doc.setFillColor(gold);doc.rect(0,footerY,210,1.2,"F");doc.setTextColor(white);doc.setFont("helvetica","bold");doc.setFontSize(7);doc.text("CLUB ROTARIO JUÁREZ CONCORDIA",11,285);doc.setFont("helvetica","normal");doc.setTextColor("#b9c7da");doc.text(`Página ${page} de ${total}`,105,286,{align:"center"});doc.addImage(motto,"PNG",158,276,40,18.8);};
  const rowsPerPage=Math.max(1,Math.floor((footerY-headerY-12)/rowHeight));
  ...
  for(let page=0;page<pages;page++){
    ...
    pageRows.forEach((record,index)=>{const y=headerY+9+index*rowHeight;if(index%2===0){doc.setFillColor(cream);doc.rect(left,y,tableWidth,rowHeight,"F");}doc.setDrawColor(line);doc.setLineWidth(.15);doc.line(left,y+rowHeight,left+tableWidth,y+rowHeight);doc.setTextColor(blue);doc.setFont("helvetica","bold");doc.setFontSize(8);doc.text(String(record.square.id),columns[0].x+columns[0].w/2,y+5.4,{align:"center"});doc.setTextColor(navy);doc.text(String(record.visitor),columns[1].x+columns[1].w/2,y+5.4,{align:"center"});doc.text(String(record.home),columns[2].x+columns[2].w/2,y+5.4,{align:"center"});doc.setFont("helvetica","normal");doc.setFontSize(7.5);doc.text(fit(record.square.participant,columns[3].w-4),columns[3].x+2,y+5.4);doc.text(fit(record.seller,columns[4].w-4),columns[4].x+2,y+5.4);});
    doc.setDrawColor("#bcc6d4");columns.slice(1).forEach((column)=>doc.line(column.x,headerY,column.x,headerY+9+pageRows.length*rowHeight));
  }
