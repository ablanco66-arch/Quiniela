...
type Status = "available" | "reserved" | "paid" | "treasury";
type Role = "admin" | "user" | "treasury";
type FlyerType = "board" | "board-en" | "schedule" | "schedule-en" | "report" | "payment" | "payment-en" | "winner" | "winner-en";
type Square = {
...
type SeasonArchive = { id:number; seasonName:string; squarePrice:number; gamePrize:number; gameCount:number; archivedAt:string };
type GameResult = { gameId:number; visitorScore:number; homeScore:number; updatedByName?:string; updatedAt?:string };
type ReportRow = { name:string; reserved:number; paid:number; treasury:number; total:number };
type ReportSortKey = keyof ReportRow;
type WinnerRow = { result:GameResult; game:Game; square:Square; visitorDigit:number; homeDigit:number; seasonName:string; gamePrize:number };
type SquareReportSortKey = "square"|"visitor"|"home"|"player"|"seller"|"status";
type DetailSortKey = "square"|"status"|"player"|"visitor";
...
export default function Home() {
  ...
  const counts = useMemo(() => ({ available:squares.filter((s) => s.status === "available").length, reserved:squares.filter((s) => s.status === "reserved").length, paid:squares.filter((s) => s.status === "paid").length, treasury:squares.filter((s) => s.status === "treasury").length }), [squares]);
  ...
  async function clearGameResult(gameId:number) {
    setSaving(true);
    try { await put({action:"game_result_clear",gameId});setGameResults((current)=>current.filter((result)=>result.gameId!==gameId));setNotice(`Resultado del juego ${gameId} eliminado`);await loadBoard();return true; }
    catch(error){setNotice(error instanceof Error?error.message:"No se pudo limpiar el resultado.");return false;}
    finally{setSaving(false);}
  }

  async function clearAllGameResults() {
    setSaving(true);
    try { const data=await put({action:"game_results_clear_all"});setGameResults([]);setNotice(`${data.cleared} resultado${data.cleared===1?"":"s"} eliminado${data.cleared===1?"":"s"}`);await loadBoard();return true; }
    catch(error){setNotice(error instanceof Error?error.message:"No se pudieron limpiar los resultados.");return false;}
    finally{setSaving(false);}
  }

  async function saveSeason(nextSeason:SeasonConfig,activate=false) {
    setSaving(true);
    try {
      const data=await put({action:activate?"season_activate":"season_save",season:nextSeason,confirmSeasonName:season.name});
      setSeason(data.season);
      await loadBoard();
      setNotice(activate?`Temporada ${data.season.name} activada; la anterior quedó archivada`:`Configuración de ${data.season.name} guardada`);
      return true;
    } catch(error){setNotice(error instanceof Error?error.message:"No se pudo guardar la temporada");return false;}
    finally{setSaving(false);}
  }
  ...
  async function openPaymentFlyer(language:"es"|"en"="es") {
    setGeneratingFlyer(true);
    try {
      const result = await createPaymentFlyer(language);
      setFlyerType(language==="en"?"payment-en":"payment"); setFlyerUrl(result.url); setFlyerBlob(result.blob); setShowFlyer(true);
    } catch { setNotice("No fue posible generar el flyer de cobro. Intenta nuevamente."); }
    finally { setGeneratingFlyer(false); }
  }

  async function openWinnerFlyer(winner:WinnerRow,language:"es"|"en"="es") {
    setGeneratingFlyer(true);
    try {
      const result = await createWinnerFlyer(winner,language);
      setFlyerType(language==="en"?"winner-en":"winner"); setFlyerUrl(result.url); setFlyerBlob(result.blob); setShowFlyer(true);
  ...
  async function copyFlyer() {
    if (!flyerBlob) return;
    try {
      await navigator.clipboard.write([new ClipboardItem({ [flyerBlob.type||"image/png"]: flyerBlob })]);
  ...
  async function shareFlyer() {
    ...
    const file = new File([flyerBlob], filename, { type:flyerBlob.type||"image/png" });
    try {
      if (navigator.share && navigator.canShare?.({ files:[file] })) await navigator.share({ title:flyerType==="winner-en"?"We have an MNF winner!":flyerType==="winner"?"¡Tenemos ganador MNF!":flyerType==="payment-en"?"Square payment · MNF Football Pool 2026":flyerType==="payment"?"Liquidación de casillas · Quiniela MNF 2026":flyerType==="board-en"?"MNF Football Pool 2026":"Quiniela MNF 2026", text:flyerType==="winner-en"?"Congratulations to our winner! Thank you for playing and supporting causes that create lasting impact.":flyerType==="winner"?"¡Felicidades a nuestro ganador! Gracias por participar y apoyar causas que generan un impacto duradero.":flyerType==="payment-en"?"Thank you for supporting this great cause!\nHere are the options to pay for your pending squares in the 2026 MNF Football Pool.":flyerType==="payment"?"¡Gracias por sumarte a esta buena causa!\nTe comparto las opciones para liquidar tus casillas pendientes de la Quiniela MNF 2026. ¡Quedo a tus órdenes!":flyerType==="report"?"¡Vamos por las 100! Este es nuestro avance de ventas en la Quiniela MNF 2026.":flyerType==="board-en"?"Play for a great cause! Pick your square in the 2026 MNF Football Pool.":"¡Participa por una buena causa! Aparta tu casilla de la Quiniela MNF 2026.", files:[file] });
  ...
  const flyerInEnglish = flyerType.endsWith("-en");

  return <main>
    <header className="topbar">
      <img className="club-logo header-logo" src="/logo-crjc-white-gold.png" alt="Rotary Juárez Concordia" />
      <div className="brand-copy"><h1>Quiniela MNF <span>{season.name}</span></h1></div>
    ...
    <section className="hero sports-hero">
      ...
      <div className="hero-copy"><span className="eyebrow">Lotería anual pro ayuda · Temporada {season.name}</span><h2>Un tablero. <em>{season.games.length} oportunidades</em> de ganar.</h2><p>Cada casilla apoya proyectos de salud, subvenciones humanitarias y el combate contra la polio.</p></div>
      <div className="hero-sports" aria-hidden="true"><div className="football"><span className="laces"><i/><i/><i/><i/></span></div><div className="scoreboard"><span>PREMIO POR JUEGO</span><strong>${season.gamePrize}</strong><small>USD · MARCADOR FINAL</small></div></div>
      <div className="goalpost" aria-hidden="true"><i/><b/><span/></div>
    </section>

    <div className="sports-strip" aria-label="Datos principales de la quiniela"><div><span className="mini-football" aria-hidden="true"/><strong>{season.games.length}</strong><small>Juegos MNF</small></div><div><span className="yard-icon" aria-hidden="true">50</span><strong>100</strong><small>Casillas</small></div><div><span className="trophy-icon" aria-hidden="true">★</span><strong>${season.squarePrice}</strong><small>Por casilla</small></div><div><span className="heart-icon" aria-hidden="true">♥</span><strong>1</strong><small>Gran causa</small></div></div>
    ...
    {tab === "board" && <section className="content board-section">
      <div className="section-heading"><div><p className="kicker">Tablero oficial · 100 casillas</p><h3>Elige tu número de la suerte</h3></div><div className="heading-actions"><button className="flyer-button" onClick={()=>openFlyer("es",true)} disabled={generatingFlyer}>{generatingFlyer ? "Generando…" : "🏈 Flier español"}</button><button className="flyer-button flyer-button-en" onClick={()=>openFlyer("en",true)} disabled={generatingFlyer}>{generatingFlyer ? "Generating…" : "🏈 English flyer"}</button><button className="flyer-button" onClick={()=>openPaymentFlyer("es")} disabled={generatingFlyer}>{generatingFlyer ? "Generando…" : "💛 Flyer Cobro"}</button><button className="flyer-button flyer-button-en" onClick={()=>openPaymentFlyer("en")} disabled={generatingFlyer}>{generatingFlyer ? "Generating…" : "💛 Payment Flyer"}</button>{me?.role === "admin" && <button className="outline-button" onClick={() => setShowDigits(true)}>⚙ Números de juego</button>}</div></div>
      <div className="summary-grid">
        ...
        <button className={filter === "treasury" ? "summary treasury-summary active" : "summary treasury-summary"} onClick={() => setFilter(filter === "treasury" ? "all" : "treasury")}><span className="dot treasury"/><div><strong>{counts.treasury}</strong><small>Tesorería</small></div></button>
        <div className="summary raised"><span className="dot goal">$</span><div><strong>${(counts.reserved + counts.paid + counts.treasury) * season.squarePrice}</strong><small>Comprometidos</small></div></div>
      </div>
      <div className="board-card"><div className="board-meta"><div className="legend"><span><i className="available"/>Disponible</span><span><i className="reserved"/>Reservada</span><span><i className="paid"/>Pagada</span><span><i className="treasury"/>Tesorería</span></div><p>{boardLocked ? "Tablero cerrado · números de juego cargados" : "Toca una casilla para ver sus datos"}</p></div>
      ...
      <div className="deadline"><span>!</span><div><strong>Fecha límite de pago</strong><p>{season.paymentDeadline?`Las casillas deben cubrirse en su totalidad antes del ${formatPaymentDeadline(season.paymentDeadline,"es")}.`:"La fecha límite de pago todavía no ha sido definida."}</p></div></div>
    </section>}

    {tab === "games" && <Games season={season} squares={squares} visitorDigits={visitorDigits} homeDigits={homeDigits} gameResults={gameResults} me={me!} saving={saving} generatingFlyer={generatingFlyer} onGenerateWinnerFlyer={openWinnerFlyer} onSaveGameResult={saveGameResult} onClearGameResult={clearGameResult} onClearAllGameResults={clearAllGameResults} />}
    {tab === "rules" && <Rules season={season} />}
    {tab === "reports" && <Reports rows={report} squares={squares} visitorDigits={visitorDigits} homeDigits={homeDigits} activity={activity} me={me!} members={members} season={season} seasonArchives={seasonArchives} saving={saving} generatingFlyer={generatingFlyer} onGenerateFlyer={openReportFlyer} onSaveMember={saveMember} onRemoveMember={removeMember} onSaveSeason={saveSeason} />}
...
function SquareModal({ square, notes, me, members, saving, boardLocked, visitorDigits, homeDigits, onClose, onSave, onMutateNote }:{ square:Square; notes:SquareNote[]; me:Member; members:Member[]; saving:boolean; boardLocked:boolean; visitorDigits:string; homeDigits:string; onClose:()=>void; onSave:(square:Square)=>void; onMutateNote:(action:NoteAction,squareId:number,text?:string,noteId?:number)=>Promise<boolean> }) {
  ...
  const statuses = (["available","reserved","paid","treasury"] as Status[]);
  const treasuryStatusChange = (status:Status) => me.role === "treasury" && status !== square.status && ["reserved","paid","treasury"].includes(square.status) && ["paid","treasury"].includes(status);
  const allowed = (status:Status) => admin || (boardLocked ? treasuryStatusChange(status) : (square.status === "available" && status === "reserved") || (owns && status === square.status) || treasuryStatusChange(status));
  ...
  return <div className="modal-backdrop square-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal square-modal" role="dialog" aria-modal="true" aria-labelledby="square-title"><button className="modal-close" onClick={onClose} aria-label="Cerrar">×</button><div className={`modal-number ${draft.status}`}>{draft.id}</div><div><p className="kicker">Detalle de casilla</p><h3 id="square-title">Casilla #{draft.id}</h3></div>
    ...
    <div className={`square-matchup ${gameNumbersReady?"":"pending"}`}><span>Números de juego</span>{gameNumbersReady?<strong><b className="visitor-number"><span>Visitante</span><em>{visitorDigit}</em></b><i>VS</i><b className="home-number"><span>Casa</span><em>{homeDigit}</em></b></strong>:<strong>Pendientes de asignación</strong>}</div>
    {draft.status !== "available" && <>{editableDetails ? <label>Nombre de quien juega<input value={draft.participant} onChange={(e) => update("participant",e.target.value)} placeholder="Nombre completo"/></label> : <div className="readonly-data"><small>JUGADOR</small><strong>{draft.participant}</strong></div>}
...
function SquareNoteLog({square,notes,me,saving,onMutate}:{square:Square;notes:SquareNote[];me:Member;saving:boolean;onMutate:(action:NoteAction,squareId:number,text?:string,noteId?:number)=>Promise<boolean>}) {
  ...
  const chronological=[...notes].sort((a,b)=>new Date(a.createdAt).getTime()-new Date(b.createdAt).getTime()||a.id-b.id);
  const noteNumbers=new Map(chronological.map((note,index)=>[note.id,index+1]));
  const ordered=sort==="desc"?[...chronological].reverse():chronological;
  ...
  return <section className="square-notes"><div className="square-notes-head"><div><strong>Bitácora de notas</strong><span>{notes.length} {notes.length===1?"registro":"registros"}</span></div><button type="button" onClick={()=>setSort((current)=>current==="desc"?"asc":"desc")} aria-label="Cambiar orden de las notas">{sort==="desc"?"Más recientes ↓":"Más antiguas ↑"}</button></div>
    ...
    <div className="note-list" aria-label="Historial de notas">{ordered.length?ordered.map((note)=>{const editable=canModify(note),editing=editingId===note.id;return <article key={note.id}><div className="note-meta"><b className="note-number">Nota No. {noteNumbers.get(note.id)} / {notes.length}</b><span className={`drilldown-status ${note.status}`}>{labelFor(note.status)}</span><strong>{note.authorName}</strong><time dateTime={note.createdAt}>{formatNoteDate(note.createdAt)}</time>{note.updatedAt!==note.createdAt&&<em>Editada</em>}</div>{editing?<><textarea value={editingText} onChange={(event)=>setEditingText(event.target.value)} rows={3} maxLength={2000}/><div className="note-edit-actions"><button type="button" onClick={()=>{setEditingId(null);setEditingText("");}}>Cancelar</button><button type="button" disabled={saving||!editingText.trim()} onClick={()=>updateNote(note.id)}>Guardar</button></div></>:<p>{note.text}</p>}{editable&&!editing&&<div className="note-actions"><button type="button" onClick={()=>{setEditingId(note.id);setEditingText(note.text);}}>Editar</button><button type="button" onClick={()=>deleteNote(note.id)}>Eliminar</button></div>}</article>}):<p className="notes-empty">Aún no hay notas en esta casilla.</p>}</div>
  </section>;
}

function Reports({ rows, squares, visitorDigits, homeDigits, activity, me, members, season, seasonArchives, saving, generatingFlyer, onGenerateFlyer, onSaveMember, onRemoveMember, onSaveSeason }:{ rows:ReportRow[]; squares:Square[]; visitorDigits:string; homeDigits:string; activity:Activity[]; me:Member; members:Member[]; season:SeasonConfig; seasonArchives:SeasonArchive[]; saving:boolean; generatingFlyer:boolean; onGenerateFlyer:()=>void; onSaveMember:(member:Member,originalEmail?:string)=>Promise<boolean>; onRemoveMember:(member:Member)=>Promise<boolean>; onSaveSeason:(season:SeasonConfig,activate?:boolean)=>Promise<boolean> }) {
  const [reportView,setReportView] = useState<"summary"|"squares"|"activity"|"access"|"season">("summary");
  ...
  const [summarySort,setSummarySort] = useState<{key:ReportSortKey;direction:"asc"|"desc"}>({key:"total",direction:"desc"});
  const [detailSort,setDetailSort] = useState<{key:DetailSortKey;direction:"asc"|"desc"}>({key:"square",direction:"asc"});
  ...
  useEffect(()=>{if(me.role!=="admin"&&(reportView==="activity"||reportView==="access"||reportView==="season"))setReportView("summary");},[me.role,reportView]);
  ...
  const totals = { reserved:rows.reduce((n,row)=>n+row.reserved,0), paid:rows.reduce((n,row)=>n+row.paid,0), treasury:rows.reduce((n,row)=>n+row.treasury,0), total:rows.reduce((n,row)=>n+row.total,0) };
  const summaryRows=[...rows].sort((a,b)=>{const av=a[summarySort.key],bv=b[summarySort.key],comparison=typeof av==="number"&&typeof bv==="number"?av-bv:String(av).localeCompare(String(bv),"es",{sensitivity:"base"});return comparison?(summarySort.direction==="asc"?comparison:-comparison):a.name.localeCompare(b.name,"es",{sensitivity:"base"});});
  const toggleSummarySort=(key:ReportSortKey)=>setSummarySort((current)=>({key,direction:current.key===key&&current.direction==="asc"?"desc":"asc"}));
  const summarySortIcon=(key:ReportSortKey)=>summarySort.key===key?(summarySort.direction==="asc"?"▲":"▼"):"↕";
  const gameNumbersReady=isDigitSet(visitorDigits)&&isDigitSet(homeDigits);
  const detailSortValue=(square:Square):string|number=>detailSort.key==="square"?square.id:detailSort.key==="status"?(["reserved","paid","treasury"] as Status[]).indexOf(square.status):detailSort.key==="player"?square.participant:(gameNumbersReady?Number(visitorDigits[(square.id-1)%10]):-1);
  const detailSquares=squares.filter((square)=>square.status!=="available"&&(square.reservedByName||square.contact||"Sin asignar")===detailName).sort((a,b)=>{const av=detailSortValue(a),bv=detailSortValue(b),comparison=typeof av==="number"&&typeof bv==="number"?av-bv:String(av).localeCompare(String(bv),"es",{sensitivity:"base"});return (detailSort.direction==="asc"?1:-1)*(comparison||(a.id-b.id));});
  const toggleDetailSort=(key:DetailSortKey)=>setDetailSort((current)=>({key,direction:current.key===key&&current.direction==="asc"?"desc":"asc"}));
  const detailSortIcon=(key:DetailSortKey)=>detailSort.key===key?(detailSort.direction==="asc"?"▲":"▼"):"↕";
  ...
  const generateDetailPdf=async()=>{setGeneratingPdf(true);try{await createSquareDetailPdf(squareReportRows,visitorDigits,homeDigits,season,squareFilter);}catch(error){window.alert(error instanceof Error?error.message:"No fue posible generar el PDF");}finally{setGeneratingPdf(false);}};
  ...
  return <section className="content reports-section"><div className="section-heading"><div><p className="kicker">Control y seguimiento</p><h3>Reportes</h3></div><span className="year-pill">{squares.filter((s)=>s.status!=="available").length} vendidas</span></div>
    <div className="report-switch"><button className={reportView==="summary"?"active":""} onClick={()=>setReportView("summary")}>Sumario por socio</button><button className={reportView==="squares"?"active":""} onClick={()=>setReportView("squares")}>Detalle de casillas</button>{me.role==="admin"&&<><button className={reportView==="activity"?"active":""} onClick={()=>setReportView("activity")}>Actividad</button><button className={reportView==="access"?"active":""} onClick={()=>setReportView("access")}>Accesos{pendingCount>0&&<b className="pending-count">{pendingCount}</b>}</button><button className={reportView==="season"?"active":""} onClick={()=>setReportView("season")}>Temporada</button></>}</div>
    {reportView === "summary" && <div className="report-card"><div className="report-card-head"><div><h4>Casillas por socio</h4><p>Selecciona el nombre de un socio para consultar el detalle de sus casillas.</p></div><div className="report-head-actions">{me.role==="admin"&&<button className="report-flyer-button" onClick={onGenerateFlyer} disabled={generatingFlyer}>{generatingFlyer?"Generando…":"🏈 Flier de avance"}</button>}<div className="report-value"><strong>${(totals.paid+totals.treasury)*100}</strong><span>cobrado</span></div></div></div><div className="report-table-wrap"><table className="report-table"><thead><tr>{([['name','Socio'],['reserved','Reservada'],['paid','Pagada'],['treasury','Tesorería'],['total','Total']] as [ReportSortKey,string][]).map(([key,label])=><th key={key} aria-sort={summarySort.key===key?(summarySort.direction==="asc"?"ascending":"descending"):"none"}><button onClick={()=>toggleSummarySort(key)}>{label}<span>{summarySortIcon(key)}</span></button></th>)}</tr></thead><tbody>{summaryRows.map((row)=><tr key={row.name}><td><button className="report-member-link" onClick={()=>{setDetailName(row.name);setDetailSort({key:"square",direction:"asc"});}}>{row.name}</button></td><td>{row.reserved || ""}</td><td>{row.paid || ""}</td><td>{row.treasury || ""}</td><td><strong>{row.total}</strong></td></tr>)}</tbody><tfoot><tr><td>Total general</td><td>{totals.reserved}</td><td>{totals.paid}</td><td>{totals.treasury}</td><td>{totals.total}</td></tr></tfoot></table></div></div>}
    {reportView === "squares" && <div className="report-card all-squares-card"><div className="report-card-head"><div><h4>Detalle de casillas reservadas y pagadas</h4><p>{squareReportRows.length} registros mostrados · el PDF respetará el filtro de socio seleccionado.</p></div><div className="square-report-actions"><button className="detail-pdf-button" onClick={generateDetailPdf} disabled={generatingPdf||!gameNumbersReady} title={gameNumbersReady?"Generar detalle en PDF":"Carga primero los números de juego"}>{generatingPdf?"Generando…":"📄 Generar PDF"}</button><label className="square-report-filter">Socio<select value={squareFilter} onChange={(event)=>setSquareFilter(event.target.value)}><option value="all">Todos los socios</option>{rows.map((row)=><option key={row.name} value={row.name}>{row.name}</option>)}</select></label></div></div><div className="all-squares-wrap"><table className="all-squares-table"><thead><tr>{([['square','Casilla'],['visitor','Visitante'],['home','Casa'],['player','Jugador'],['seller','Socio'],['status','Estado']] as [SquareReportSortKey,string][]).map(([key,label])=><th key={key} aria-sort={squareSort.key===key?(squareSort.direction==="asc"?"ascending":"descending"):"none"}><button onClick={()=>toggleSquareSort(key)}>{label}<span>{squareSortIcon(key)}</span></button></th>)}</tr></thead><tbody>{squareReportRows.map((square)=>{const visitor=gameNumbersReady?visitorDigits[(square.id-1)%10]:"—",home=gameNumbersReady?homeDigits[Math.floor((square.id-1)/10)]:"—",seller=squareSeller(square);return <tr key={square.id}><td><strong>#{square.id}</strong></td><td>{visitor}</td><td>{home}</td><td>{square.participant||"—"}</td><td>{seller||"—"}</td><td><span className={`drilldown-status ${square.status}`}>{labelFor(square.status)}</span></td></tr>;})}</tbody></table></div></div>}
    ...
    {reportView === "access" && <div className="access-management"><div className="member-form report-card"><div className="report-card-head"><div><h4>{editingEmail ? "Editar acceso" : "Crear cuenta propia"}</h4><p>{draft.approvalStatus==="pending"?"Revisa la solicitud, asigna el nivel y aprueba al nuevo usuario.":editingEmail?"Actualiza el nivel, estado o asigna una nueva contraseña temporal.":"Crea el usuario y comparte la contraseña temporal de forma privada."}</p></div>{editingEmail && <button className="form-cancel" onClick={resetMemberForm}>Cancelar</button>}</div><div className="member-form-grid">
      ...
      {(!editingEmail || draft.username) && <label>{editingEmail?"Nueva contraseña temporal (opcional)":"Contraseña temporal"}<div className="password-reveal-control"><input value={draft.tempPassword??""} onChange={(e)=>setDraft({...draft,tempPassword:e.target.value})} type={showTempPassword?"text":"password"} autoComplete="new-password" minLength={8} maxLength={128} pattern={PASSWORD_PATTERN} title={PASSWORD_HINT} placeholder="Contraseña segura"/><button type="button" className={showTempPassword?"revealing":""} aria-label="Mantén presionado para mostrar la contraseña" aria-pressed={showTempPassword} title="Mantén presionado para mostrar" onPointerDown={(event)=>{event.preventDefault();setShowTempPassword(true);}} onPointerUp={()=>setShowTempPassword(false)} onPointerCancel={()=>setShowTempPassword(false)} onPointerLeave={()=>setShowTempPassword(false)} onBlur={()=>setShowTempPassword(false)} onKeyDown={(event)=>{if(event.key===" "||event.key==="Enter"){event.preventDefault();setShowTempPassword(true);}}} onKeyUp={(event)=>{if(event.key===" "||event.key==="Enter")setShowTempPassword(false);}} onContextMenu={(event)=>event.preventDefault()}><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M2.4 12s3.5-5.5 9.6-5.5S21.6 12 21.6 12 18.1 17.5 12 17.5 2.4 12 2.4 12Z"/><circle cx="12" cy="12" r="2.7"/></svg></button></div><small className="password-hint">{PASSWORD_HINT}</small></label>}
      ...
      <div className="report-card member-list"><div className="report-card-head"><div><h4>Usuarios y solicitudes</h4><p>{members.filter((member)=>member.active).length} activos · {pendingCount} pendientes</p></div></div>{members.map((member)=>{const pending=member.approvalStatus==="pending";return <article className={pending?"member-pending":""} key={member.email}><div><strong>{member.name}</strong><span>{member.username?`@${member.username}`:member.email}</span></div><span className={pending?"member-awaiting":member.active?"member-active":"member-inactive"}>{pending?"Pendiente":member.active?roleLabel(member.role):"Suspendido"}</span><div className="member-actions">{pending&&<button className="member-approve" disabled={saving} onClick={()=>onSaveMember({...member,active:1,approvalStatus:"approved"},member.email)}>Aprobar</button>}<button className="member-edit" onClick={()=>{setDraft({...member});setEditingEmail(member.email);setShowTempPassword(false);}}>Editar</button><button className="member-remove" disabled={member.email===me.email||saving} onClick={async()=>{if(window.confirm(`${pending?"¿Rechazar la solicitud":"¿Retirar el acceso"} de ${member.name}?`)&&await onRemoveMember(member)){if(editingEmail===member.email)resetMemberForm();}}}>{pending?"Rechazar":"Retirar"}</button></div></article>})}</div>
    </div>}
    {me.role==="admin"&&reportView==="season"&&<SeasonManager season={season} archives={seasonArchives} saving={saving} onSave={onSaveSeason}/>}
    {detailName && <div className="modal-backdrop" role="presentation" onMouseDown={(event)=>event.target===event.currentTarget&&setDetailName("")}><section className="modal drilldown-modal" role="dialog" aria-modal="true" aria-labelledby="drilldown-title"><button className="modal-close" onClick={()=>setDetailName("")} aria-label="Cerrar">×</button><p className="kicker">Detalle por socio</p><h3 id="drilldown-title">{detailName}</h3><p className="drilldown-summary">{detailSquares.length} casillas · {detailSquares.filter((square)=>square.status==="reserved").length} reservadas · {detailSquares.filter((square)=>square.status==="paid").length} pagadas · {detailSquares.filter((square)=>square.status==="treasury").length} en Tesorería</p><div className="drilldown-table-wrap"><table className="drilldown-table"><thead><tr><th aria-sort={detailSort.key==="square"?(detailSort.direction==="asc"?"ascending":"descending"):"none"}><button onClick={()=>toggleDetailSort("square")}>Casilla <span>{detailSortIcon("square")}</span></button></th><th aria-sort={detailSort.key==="status"?(detailSort.direction==="asc"?"ascending":"descending"):"none"}><button onClick={()=>toggleDetailSort("status")}>Estado <span>{detailSortIcon("status")}</span></button></th><th aria-sort={detailSort.key==="player"?(detailSort.direction==="asc"?"ascending":"descending"):"none"}><button onClick={()=>toggleDetailSort("player")}>Jugador <span>{detailSortIcon("player")}</span></button></th><th aria-sort={detailSort.key==="visitor"?(detailSort.direction==="asc"?"ascending":"descending"):"none"}><button onClick={()=>toggleDetailSort("visitor")}>Visitante <span>{detailSortIcon("visitor")}</span></button></th><th>Casa</th></tr></thead><tbody>{detailSquares.map((square)=>{const visitor=gameNumbersReady?visitorDigits[(square.id-1)%10]:"—",home=gameNumbersReady?homeDigits[Math.floor((square.id-1)/10)]:"—";return <tr key={square.id}><td><strong>#{square.id}</strong></td><td><span className={`drilldown-status ${square.status}`}>{labelFor(square.status)}</span></td><td>{square.participant||"—"}</td><td>{visitor}</td><td>{home}</td></tr>;})}</tbody></table></div></section></div>}
  </section>;
}

function SeasonManager({season,archives,saving,onSave}:{season:SeasonConfig;archives:SeasonArchive[];saving:boolean;onSave:(season:SeasonConfig,activate?:boolean)=>Promise<boolean>}){
  const [draft,setDraft]=useState<SeasonConfig>(season);
  useEffect(()=>setDraft(season),[season]);
  const updateGame=(index:number,field:keyof Game,value:string)=>setDraft((current)=>({...current,games:current.games.map((game,i)=>i===index?{...game,[field]:value}:game)}));
  const moveGame=(index:number,direction:-1|1)=>setDraft((current)=>{const target=index+direction;if(target<0||target>=current.games.length)return current;const games=[...current.games];[games[index],games[target]]=[games[target],games[index]];return{...current,games};});
  const valid=draft.name.trim().length>=2&&draft.squarePrice>0&&draft.gamePrize>0&&draft.games.length>0&&draft.games.every((game)=>game.date.trim()&&game.visitor.trim()&&game.home.trim());
  const changed=JSON.stringify(draft)!==JSON.stringify(season);
  async function activate(){if(!valid)return;const message=`Se archivará la temporada ${season.name} y se limpiarán las 100 casillas, los números de juego y los resultados para activar ${draft.name}. Los usuarios se conservarán.\n\n¿Deseas continuar?`;if(window.confirm(message))await onSave(draft,true);}
  return <div className="season-manager"><div className="report-card season-intro"><div><p className="kicker">Preparación anual</p><h4>Configuración de temporada</h4><p>Edita el calendario durante la preparación. Guardar conserva el tablero actual; activar crea una copia histórica y deja el tablero listo para comenzar.</p></div><span className="season-current">ACTIVA · {season.name}</span></div>
    <div className="report-card season-form"><div className="season-fields"><label>Nombre de temporada<input value={draft.name} maxLength={24} onChange={(event)=>setDraft({...draft,name:event.target.value})} placeholder="Ej. 2027"/></label><label>Costo por casilla (USD)<input type="number" min="1" value={draft.squarePrice} onChange={(event)=>setDraft({...draft,squarePrice:Number(event.target.value)})}/></label><label>Premio por juego (USD)<input type="number" min="1" value={draft.gamePrize} onChange={(event)=>setDraft({...draft,gamePrize:Number(event.target.value)})}/></label><label>Fecha límite de pago <span>(opcional)</span><input type="date" value={draft.paymentDeadline} onChange={(event)=>setDraft({...draft,paymentDeadline:event.target.value})}/></label></div>
      <div className="season-games-head"><div><h4>Juegos participantes</h4><p>{draft.games.length} juegos · el orden define el número de juego usado en resultados.</p></div><button type="button" className="outline-button" disabled={draft.games.length>=25} onClick={()=>setDraft({...draft,games:[...draft.games,{date:"",visitor:"",home:""}]})}>+ Agregar juego</button></div>
      <div className="season-games">{draft.games.map((game,index)=><article key={index}><strong>{String(index+1).padStart(2,"0")}</strong><label>Fecha<input value={game.date} onChange={(event)=>updateGame(index,"date",event.target.value)} placeholder="Ej. 13 sep"/></label><label>Visitante<input value={game.visitor} onChange={(event)=>updateGame(index,"visitor",event.target.value)} placeholder="Equipo visitante"/></label><label>Casa<input value={game.home} onChange={(event)=>updateGame(index,"home",event.target.value)} placeholder="Equipo de casa"/></label><div className="season-row-actions"><button type="button" disabled={index===0} onClick={()=>moveGame(index,-1)} aria-label="Subir juego">↑</button><button type="button" disabled={index===draft.games.length-1} onClick={()=>moveGame(index,1)} aria-label="Bajar juego">↓</button><button type="button" className="remove" disabled={draft.games.length===1} onClick={()=>setDraft({...draft,games:draft.games.filter((_,i)=>i!==index)})} aria-label="Eliminar juego">×</button></div></article>)}</div>
      <div className="season-actions"><button className="secondary" disabled={saving||!changed} onClick={()=>setDraft(season)}>Descartar cambios</button><button className="primary" disabled={saving||!valid||!changed} onClick={()=>onSave(draft,false)}>{saving?"Guardando…":"Guardar configuración"}</button><button className="activate-season" disabled={saving||!valid||draft.name.trim()===season.name.trim()} onClick={activate}>Activar nueva temporada</button></div>
    </div>
    <div className="report-card season-history"><div className="report-card-head"><div><h4>Temporadas archivadas</h4><p>Copias históricas creadas al activar una temporada nueva.</p></div></div>{archives.length?<div className="season-history-list">{archives.map((archive)=><article key={archive.id}><div><strong>Temporada {archive.seasonName}</strong><span>{archive.gameCount} juegos · ${archive.squarePrice} casilla · ${archive.gamePrize} premio</span></div><time>{formatDate(archive.archivedAt)}</time></article>)}</div>:<p className="empty-report">La primera copia aparecerá cuando actives la siguiente temporada.</p>}</div>
  </div>;
}

function Games({season,squares,visitorDigits,homeDigits,gameResults,me,saving,generatingFlyer,onGenerateWinnerFlyer,onSaveGameResult,onClearGameResult,onClearAllGameResults}:{season:SeasonConfig;squares:Square[];visitorDigits:string;homeDigits:string;gameResults:GameResult[];me:Member;saving:boolean;generatingFlyer:boolean;onGenerateWinnerFlyer:(winner:WinnerRow,language?:"es"|"en")=>Promise<void>;onSaveGameResult:(gameId:number,visitorScore:number,homeScore:number)=>Promise<boolean>;onClearGameResult:(gameId:number)=>Promise<boolean>;onClearAllGameResults:()=>Promise<boolean>}){
  const [gamesView,setGamesView]=useState<"calendar"|"winners">("calendar");
  ...
  const gameNumbersReady=isDigitSet(visitorDigits)&&isDigitSet(homeDigits);
  const winnerFromResult=(result:GameResult):WinnerRow|undefined=>{if(!gameNumbersReady)return;const visitorDigit=result.visitorScore%10,homeDigit=result.homeScore%10,column=visitorDigits.indexOf(String(visitorDigit)),row=homeDigits.indexOf(String(homeDigit)),square=column>=0&&row>=0?squares[row*10+column]:undefined,game=season.games[result.gameId-1];return square&&game?{result,game,square,visitorDigit,homeDigit,seasonName:season.name,gamePrize:season.gamePrize}:undefined;};
  const winnerRows=gameResults.map(winnerFromResult).filter((winner):winner is WinnerRow=>Boolean(winner));
  const selectedResultGame=season.games[resultGameId-1];
  const selectedResultExists=gameResults.some((result)=>result.gameId===resultGameId);
  return <section className="content games-section"><div className="section-heading"><div><p className="kicker">Calendario oficial</p><h3>{season.games.length} lunes de emoción</h3></div><span className="year-pill">{season.name}</span></div>
    <div className="report-switch games-switch"><button className={gamesView==="calendar"?"active":""} onClick={()=>setGamesView("calendar")}>Juegos</button><button className={gamesView==="winners"?"active":""} onClick={()=>setGamesView("winners")}>Ganadores</button></div>
    {gamesView==="calendar"&&<div className="games-list">{season.games.map((game,index)=><article className="game" key={`${game.date}-${game.visitor}-${index}`}><div className="game-number"><span>JUEGO</span><strong>{String(index+1).padStart(2,"0")}</strong></div><div className="game-date">{game.date}</div><div className="matchup"><div><small>VISITANTE</small><strong>{game.visitor}</strong></div><span>@</span><div><small>CASA</small><strong>{game.home}</strong></div></div><div className="monday">MNF</div></article>)}</div>}
    {gamesView==="winners"&&<div className="winners-view">
      {me.role==="admin"&&<form className="report-card result-entry" onSubmit={async(event)=>{event.preventDefault();const result={gameId:resultGameId,visitorScore:Number(visitorScore),homeScore:Number(homeScore)};if(await onSaveGameResult(result.gameId,result.visitorScore,result.homeScore)){const winner=winnerFromResult(result);if(winner)await onGenerateWinnerFlyer(winner);}}}>
        <div><p className="kicker">Captura administrativa</p><h4>Registrar resultado final</h4><p>Incluye el marcador después de tiempos extras. Al guardar, se generará el flier del ganador.</p></div>
        <label>Juego<select value={resultGameId} onChange={(event)=>setResultGameId(Number(event.target.value))}>{season.games.map((game,index)=><option key={index+1} value={index+1}>{String(index+1).padStart(2,"0")} · {gameDateLabel(index+1,season.games,season.name)} · {game.visitor} vs {game.home}</option>)}</select></label>
        <div className="score-entry"><label><span className="score-team">{selectedResultGame?.visitor||"Visitante"}</span><input aria-label={`Marcador final de ${selectedResultGame?.visitor||"visitante"}`} type="number" min="0" max="999" inputMode="numeric" placeholder="0" required value={visitorScore} onChange={(event)=>setVisitorScore(event.target.value)}/></label><span className="score-vs">VS</span><label><span className="score-team">{selectedResultGame?.home||"Casa"}</span><input aria-label={`Marcador final de ${selectedResultGame?.home||"casa"}`} type="number" min="0" max="999" inputMode="numeric" placeholder="0" required value={homeScore} onChange={(event)=>setHomeScore(event.target.value)}/></label></div>
        <div className="result-actions"><button type="submit" className="primary" disabled={saving||generatingFlyer||visitorScore===""||homeScore===""}>{saving?"Procesando…":generatingFlyer?"Generando…":"Guardar y generar flier"}</button><button type="button" className="clear-one-result" disabled={saving||!selectedResultExists} onClick={async()=>{if(window.confirm(`¿Eliminar el marcador del juego ${resultGameId}? Esta acción no afecta las casillas.`))await onClearGameResult(resultGameId);}}>Limpiar este juego</button><button type="button" className="clear-all-results" disabled={saving||gameResults.length===0} onClick={async()=>{if(window.confirm(`¿Eliminar los ${gameResults.length} marcadores registrados? Las casillas, pagos y números del tablero se conservarán.`))await onClearAllGameResults();}}>Limpiar todos</button></div>
      </form>}
      <div className="report-card winners-card"><div className="report-card-head"><div><h4>Casillas ganadoras</h4><p>Se utiliza la unidad del marcador final de visitante y casa, incluidos tiempos extras.</p></div><span className="winner-count">{winnerRows.length} de {season.games.length}</span></div>{!gameNumbersReady&&gameResults.length>0?<p className="empty-report">Carga los números de juego para poder determinar las casillas ganadoras.</p>:winnerRows.length===0?<p className="empty-report">Los ganadores aparecerán al registrar los resultados finales.</p>:<div className="winners-table-wrap"><table className="winners-table"><thead><tr><th>Fecha del juego</th><th>Casilla</th><th>Estado</th><th>Jugador</th><th>Resultado del juego</th><th>Visitante</th><th>Casa</th><th>Socio</th>{me.role==="admin"&&<th>Flyers</th>}</tr></thead><tbody>{winnerRows.map((winner)=><tr key={winner.result.gameId}><td>{gameDateLabel(winner.result.gameId,season.games,season.name)}</td><td><strong>#{winner.square.id}</strong></td><td><span className={`drilldown-status ${winner.square.status}`}>{labelFor(winner.square.status)}</span></td><td>{winner.square.participant||"—"}</td><td><span className="final-score"><b>{winner.game.visitor} {winner.result.visitorScore}</b><i>vs</i><b>{winner.game.home} {winner.result.homeScore}</b></span></td><td>{winner.visitorDigit}</td><td>{winner.homeDigit}</td><td>{winner.square.reservedByName||winner.square.contact||"—"}</td>{me.role==="admin"&&<td><div className="winner-flyer-actions"><button className="winner-flyer-button" disabled={generatingFlyer} onClick={()=>onGenerateWinnerFlyer(winner,"es")} aria-label="Generar flyer del ganador en español">ES</button><button className="winner-flyer-button winner-flyer-button-en" disabled={generatingFlyer} onClick={()=>onGenerateWinnerFlyer(winner,"en")} aria-label="Generate winner flyer in English">EN</button></div></td>}</tr>)}</tbody></table></div>}</div>
    </div>}
  </section>;
}

function Rules({season}:{season:SeasonConfig}){ const deadline=season.paymentDeadline?`El apoyo debe cubrirse totalmente antes del ${formatPaymentDeadline(season.paymentDeadline,"es")}.`:"La fecha límite de pago está por definir.";const rules=[[`$${season.squarePrice} USD por casilla`,deadline],[`$${season.gamePrize} USD por juego`,`Puedes ganar cada vez que tu marcador resulte premiado durante los ${season.games.length} juegos.`],["Los números se revelan al inicio","Las casillas se eligen al azar. Los dígitos permanecen ocultos hasta iniciar la temporada."],["Cuenta el marcador final","Se consideran tiempos extras y solamente la unidad del resultado de cada equipo."]]; return <section className="content rules-section"><div className="section-heading"><div><p className="kicker">Cómo se juega</p><h3>Reglas claras, diversión grande</h3></div></div><div className="rules-grid">{rules.map(([title,body],index)=><article key={title}><span>{index+1}</span><div><h4>{title}</h4><p>{body}</p></div></article>)}</div><div className="example"><span className="example-tag">EJEMPLO</span><h4>Visitante 17 — Casa 10</h4><p>Gana la casilla donde la columna <strong>7</strong> cruza con el renglón <strong>0</strong>.</p><div className="score-example"><div><small>VISITANTE</small><strong>17</strong></div><span>→</span><div className="winning-square"><small>CASILLA</small><strong>7 × 0</strong></div><span>←</span><div><small>CASA</small><strong>10</strong></div></div></div><div className="warnings"><p>Las casillas no pagadas totalmente no juegan.</p><p>Si gana una casilla no vendida, el premio se queda en el club.</p><p>Solo participan los juegos aquí listados.</p></div></section>; }

function buildReportRows(squares:Square[]):ReportRow[]{
  const rows=new Map<string,ReportRow>();
  squares.filter((square)=>square.status!=="available").forEach((square)=>{const name=square.reservedByName||square.contact||"Sin asignar";const row=rows.get(name)??{name,reserved:0,paid:0,treasury:0,total:0};if(square.status==="reserved")row.reserved+=1;else if(square.status==="paid")row.paid+=1;else row.treasury+=1;row.total+=1;rows.set(name,row);});
  return [...rows.values()].sort((a,b)=>b.total-a.total||a.name.localeCompare(b.name));
}

async function createWinnerFlyer({result,game,square,visitorDigit,homeDigit,seasonName,gamePrize}:WinnerRow,language:"es"|"en"="es"){
  const canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1920;
  const ctx=canvas.getContext("2d");if(!ctx)throw new Error("Canvas no disponible");
  const styles=getComputedStyle(document.body),sportFont=styles.getPropertyValue("--font-sport").trim()||"Impact",bodyFont=styles.getPropertyValue("--font-body").trim()||"Arial";
  await document.fonts.ready;await document.fonts.load(`400 78px ${sportFont}`);
  const navy="#061b3e",gold="#f7b500",cream="#fff8e5",white="#ffffff",red="#d7193f";
  const english=language==="en",copy=english?{name:"MNF FOOTBALL POOL",headline:"WE HAVE A WINNER!",support:"YOUR SUPPORT MAKES GREAT CAUSES POSSIBLE",congratulations:"CONGRATULATIONS",defaultWinner:"WINNING SQUARE",prize:"PRIZE",finalScore:"FINAL SCORE · OVERTIME INCLUDED",visitor:"VISITOR",home:"HOME",square:"SQUARE",soldBy:"SQUARE SOLD BY",thanks:"THANK YOU FOR PLAYING, WINNING AND HELPING!",impact:"Every square strengthens projects that transform lives."}:{name:"QUINIELA MNF",headline:"¡TENEMOS GANADOR!",support:"TU PARTICIPACIÓN HACE POSIBLE GRANDES CAUSAS",congratulations:"FELICIDADES",defaultWinner:"CASILLA GANADORA",prize:"PREMIO",finalScore:"MARCADOR FINAL · INCLUYE TIEMPOS EXTRAS",visitor:"VISITANTE",home:"CASA",square:"CASILLA",soldBy:"CASILLA COLOCADA POR",thanks:"¡GRACIAS POR JUGAR, GANAR Y AYUDAR!",impact:"Cada casilla fortalece proyectos que transforman vidas."};
  const [stadium,clubLogo,mnfLogo,motto]=await Promise.all([loadCanvasImage("/flyer-stadium-bg.png"),loadCanvasImage("/logo-crjc-white-gold.png"),loadCanvasImage("/logo-monday-night-football.png"),loadCanvasImage(english?"/rotary-motto-2026-2027-en.png":"/lema-rotario-2026-2027.png")]);
  ...
  ctx.save();const fieldTop=300,fieldBottom=1775,fieldLeft=45,fieldRight=1035;ctx.beginPath();ctx.rect(fieldLeft,fieldTop,fieldRight-fieldLeft,fieldBottom-fieldTop);ctx.clip();const turf=ctx.createLinearGradient(fieldLeft,0,fieldRight,0);turf.addColorStop(0,"#174f2d18");turf.addColorStop(.5,"#23824636");turf.addColorStop(1,"#174f2d18");ctx.fillStyle=turf;ctx.fillRect(fieldLeft,fieldTop,fieldRight-fieldLeft,fieldBottom-fieldTop);const yardStep=(fieldRight-fieldLeft)/10;for(let strip=0;strip<10;strip++){ctx.fillStyle=strip%2===0?"#43a85a0d":"#0c4d2816";ctx.fillRect(fieldLeft+strip*yardStep,fieldTop,yardStep,fieldBottom-fieldTop);}ctx.globalAlpha=.34;ctx.strokeStyle=white;ctx.fillStyle=white;ctx.lineCap="butt";ctx.textAlign="center";ctx.lineWidth=5;[fieldTop+10,fieldBottom-10].forEach((y)=>{ctx.beginPath();ctx.moveTo(fieldLeft,y);ctx.lineTo(fieldRight,y);ctx.stroke();});const numberedYards=new Map([[1,"30"],[3,"40"],[5,"50"],[7,"40"],[9,"30"]]);for(let index=0;index<=10;index++){const x=fieldLeft+index*yardStep;ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(x,fieldTop);ctx.lineTo(x,fieldBottom);ctx.stroke();const label=numberedYards.get(index);if(label){ctx.font=`400 42px ${sportFont}`;const leftDigit=label[0],rightDigit=label[1],digitOffset=21;ctx.fillText(leftDigit,x-digitOffset,fieldTop+67);ctx.fillText(rightDigit,x+digitOffset,fieldTop+67);ctx.fillText(leftDigit,x-digitOffset,fieldBottom-32);ctx.fillText(rightDigit,x+digitOffset,fieldBottom-32);}}const fieldHeight=fieldBottom-fieldTop,hashInset=17,hashSpan=fieldHeight-hashInset*2,hashRows=[0,1,2,3].map((row)=>fieldTop+hashInset+row*(hashSpan/3));hashRows.forEach((y)=>{for(let mark=1;mark<50;mark++){if(mark%5===0)continue;const x=fieldLeft+mark*((fieldRight-fieldLeft)/50);ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(x,y-10);ctx.lineTo(x,y+10);ctx.stroke();}});ctx.restore();
  ...
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 28px ${sportFont}`;ctx.fillText(`${copy.name} ${seasonName}`,540,276);ctx.fillStyle=white;canvasFontToFit(ctx,copy.headline,940,78,sportFont,48);ctx.strokeStyle=navy;ctx.lineWidth=9;ctx.strokeText(copy.headline,540,363);ctx.fillText(copy.headline,540,363);ctx.fillStyle=cream;canvasFontToFit(ctx,copy.support,900,25,sportFont,18);ctx.fillText(copy.support,540,410);
  ...
  roundedBox(ctx,350,438,380,54,27,gold,null,0);ctx.fillStyle=navy;ctx.font=`400 23px ${sportFont}`;ctx.fillText(`${winnerGameDate(game.date,language)} ${seasonName}`.toUpperCase(),540,474);
  roundedBox(ctx,86,528,908,270,26,"#051631d9",gold,4);ctx.fillStyle=gold;ctx.font=`400 27px ${sportFont}`;ctx.fillText(copy.congratulations,540,606);ctx.fillStyle=white;const winnerName=(square.participant||copy.defaultWinner).toUpperCase();canvasFontToFit(ctx,winnerName,800,74,sportFont,35);ctx.strokeStyle=navy;ctx.lineWidth=11;ctx.strokeText(winnerName,540,704);ctx.fillText(winnerName,540,704);ctx.fillStyle=gold;ctx.fillRect(250,752,580,5);
  ...
  ctx.save();ctx.shadowColor="#f7b50066";ctx.shadowBlur=36;ctx.fillStyle=gold;ctx.beginPath();ctx.arc(540,940,119,0,Math.PI*2);ctx.fill();ctx.restore();ctx.fillStyle=navy;ctx.font=`400 27px ${sportFont}`;ctx.fillText(copy.prize,540,907);ctx.font=`400 67px ${sportFont}`;ctx.fillText(`$${gamePrize}`,540,971);ctx.font=`400 23px ${sportFont}`;ctx.fillText("USD",540,1006);
  roundedBox(ctx,68,1090,944,252,25,"#06152ff2",white,2);ctx.fillStyle=red;ctx.fillRect(68,1090,944,10);ctx.fillStyle="#9fb0c7";ctx.font=`800 18px ${bodyFont}`;ctx.fillText(copy.finalScore,540,1136);
  const scoreTeams=[[game.visitor,result.visitorScore,copy.visitor,290],[game.home,result.homeScore,copy.home,790]] as const;scoreTeams.forEach(([team,score,side,x])=>{ctx.fillStyle=gold;ctx.font=`400 18px ${sportFont}`;ctx.fillText(side,x,1181);ctx.fillStyle=white;canvasFontToFit(ctx,team.toUpperCase(),350,27,sportFont,15);ctx.fillText(team.toUpperCase(),x,1225);ctx.fillStyle=gold;ctx.font=`400 70px ${sportFont}`;ctx.fillText(String(score),x,1300);});ctx.fillStyle="#8fa0b8";ctx.font=`400 26px ${sportFont}`;ctx.fillText("VS",540,1252);
...
function winnerGameDate(date:string,language:"es"|"en"){
  ...
  return date.replace(/\b(ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic)\b/gi,(month)=>months[month.toLowerCase()]||month);
}

async function createPaymentFlyer(language:"es"|"en"="es"){
  const canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1920;
  const ctx=canvas.getContext("2d");if(!ctx)throw new Error("Canvas no disponible");
  ...
  const navy="#061b3e",deepNavy="#020a1d",gold="#f7b500",cream="#fff8e5",white="#ffffff",red="#ed1b2f",green="#168553";
  const english=language==="en",copy=english?{name:"MNF FOOTBALL POOL 2026",headline1:"TOGETHER, WE REACH",headline2:"OUR GOAL!",intro:"Your contribution turns every square into support for a great cause.",payTitle:"HELP US COMPLETE PAYMENT",paySubtitle:"FOR YOUR PENDING SQUARES",cash:"CASH",or:"OR",perSquare:"PER SQUARE",transfer:"BANK TRANSFER",clabe:"BBVA CLABE",payTo:"PAY TO",qrTitle:"PAY WITH VENMO",qrAmount:"$100 USD",qrAccount:"@Yamel-Guillen",scan1:"SCAN WITH VENMO",scan2:"OR YOUR PHONE CAMERA",confirmation:"AFTER PAYING, PLEASE SHARE YOUR PAYMENT CONFIRMATION",reminder:"DON'T MISS THE DEADLINE!",deadline:"PAYMENT DEADLINE: SEPTEMBER 13",numbers1:"The numbers assigned to each square will be shared",numbers2:"on Monday, September 14.",cause1:"WE TEAM UP TO",cause2:"DO GOOD",causeBody:"Your participation strengthens Rotary's service and the fight to end polio.",teams:"MNF 2026 TEAMS",thanks:"THANK YOU FOR CREATING LASTING IMPACT"}:{name:"QUINIELA MNF 2026",headline1:"¡JUNTOS LLEGAMOS",headline2:"A LA META!",intro:"Tu cooperación convierte cada casilla en apoyo para una buena causa.",payTitle:"AYÚDANOS A LIQUIDAR",paySubtitle:"LAS CASILLAS QUE TENGAS PENDIENTES",cash:"EN EFECTIVO",or:"O BIEN",perSquare:"POR CASILLA",transfer:"TRANSFERENCIA",clabe:"CLABE BBVA",payTo:"A NOMBRE DE",qrTitle:"PAGA CON CoDi",qrAmount:"$1,800 MXN",qrAccount:"",scan1:"ESCANEA EL QR CON",scan2:"TU APP BANCARIA",confirmation:"AL PAGAR, COMPARTE TU CONFIRMACIÓN DE PAGO",reminder:"¡QUE NO SE TE PASE!",deadline:"FECHA LÍMITE DE PAGO: 13 DE SEPTIEMBRE",numbers1:"Los números con los que jugarán las casillas se compartirán",numbers2:"el lunes 14 de septiembre.",cause1:"COOPERAMOS PARA",cause2:"HACER EL BIEN",causeBody:"Tu participación fortalece el trabajo solidario de Rotary y la lucha para poner fin a la polio.",teams:"EQUIPOS MNF 2026",thanks:"GRACIAS POR GENERAR UN IMPACTO DURADERO"};
  const [stadium,clubLogo,motto,endPolio,mnfLogo,paymentQr,...teamLogos]=await Promise.all([loadCanvasImage("/flyer-stadium-bg.png"),loadCanvasImage("/logo-crjc-white-gold.png"),loadCanvasImage(english?"/rotary-motto-2026-2027-en.png":"/lema-rotario-2026-2027.png"),loadCanvasImage("/end-polio-now.png"),loadCanvasImage("/logo-monday-night-football.png"),loadCanvasImage(english?"/qr-venmo-yamel-guillen.jpg":"/qr-codi-quiniela-mnf.jpeg"),...['den','kc','nyg','lar','phi','chi','atl','no','buf','wsh','sf','dal'].map((team)=>loadCanvasImage(`/nfl-logos/${team}.png`))]);
  ctx.drawImage(stadium,0,0,1080,1920);const shade=ctx.createLinearGradient(0,0,0,1920);shade.addColorStop(0,"#020a1df2");shade.addColorStop(.52,"#061b3ee8");shade.addColorStop(1,"#020918f7");ctx.fillStyle=shade;ctx.fillRect(0,0,1080,1920);ctx.fillStyle=gold;ctx.fillRect(0,0,1080,14);ctx.fillRect(0,1906,1080,14);
  ctx.save();ctx.globalAlpha=.14;ctx.strokeStyle=white;ctx.lineWidth=2;for(let y=540;y<1810;y+=108){ctx.beginPath();ctx.moveTo(24,y);ctx.lineTo(1056,y);ctx.stroke();for(let x=70;x<1040;x+=48){ctx.beginPath();ctx.moveTo(x,y-8);ctx.lineTo(x,y+8);ctx.stroke();}}ctx.setLineDash([18,14]);ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(540,526);ctx.lineTo(540,1810);ctx.stroke();ctx.setLineDash([]);ctx.font=`400 18px ${displayFont}`;ctx.textAlign="center";[10,20,30,40,50,40,30,20,10,20,30,40].forEach((yard,index)=>{const y=570+index*108;ctx.fillStyle=white;ctx.fillText(String(yard),42,y);ctx.fillText(String(yard),1038,y);});ctx.restore();
  const clubW=430,clubH=clubW*(clubLogo.height/clubLogo.width);ctx.drawImage(clubLogo,72,34,clubW,clubH);const mnfW=168,mnfH=mnfW*(mnfLogo.height/mnfLogo.width);ctx.drawImage(mnfLogo,840,48,mnfW,mnfH);
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 28px ${displayFont}`;ctx.fillText(copy.name,540,302);ctx.fillStyle=white;ctx.font=`400 67px ${displayFont}`;ctx.fillText(copy.headline1,540,382);ctx.fillText(copy.headline2,540,450);ctx.fillStyle=cream;ctx.font=`700 27px ${bodyFont}`;canvasFontToFit(ctx,copy.intro,930,27,bodyFont,21);ctx.fillText(copy.intro,540,500);
  roundedBox(ctx,68,542,944,112,24,gold,null,0);ctx.fillStyle=navy;ctx.font=`400 31px ${displayFont}`;ctx.fillText(copy.payTitle,540,588);ctx.font=`800 24px ${bodyFont}`;ctx.fillText(copy.paySubtitle,540,626);
  roundedBox(ctx,68,688,294,380,24,"#071a3bef",gold,3);ctx.fillStyle=gold;ctx.font=`400 25px ${displayFont}`;ctx.fillText(copy.cash,215,740);ctx.fillStyle=white;ctx.font=`400 48px ${displayFont}`;ctx.fillText("$100 USD",215,835);ctx.fillStyle="#b9c7da";ctx.font=`800 18px ${bodyFont}`;ctx.fillText(copy.or,215,882);ctx.fillStyle=white;ctx.font=`400 39px ${displayFont}`;ctx.fillText("$1,800 MXN",215,950);ctx.fillStyle=gold;ctx.font=`800 17px ${bodyFont}`;ctx.fillText(copy.perSquare,215,1002);
  roundedBox(ctx,393,688,294,380,24,"#071a3bef",gold,3);ctx.fillStyle=gold;canvasFontToFit(ctx,copy.transfer,260,23,displayFont,18);ctx.fillText(copy.transfer,540,740);ctx.fillStyle=white;ctx.font=`400 35px ${displayFont}`;ctx.fillText("$1,800 MXN",540,795);ctx.font=`800 18px ${bodyFont}`;ctx.fillText(copy.clabe,540,848);ctx.font=`400 19px ${displayFont}`;ctx.fillText("012180015213044882",540,890);ctx.fillStyle="#b9c7da";ctx.font=`700 16px ${bodyFont}`;ctx.fillText(copy.payTo,540,948);ctx.fillStyle=white;ctx.font=`400 24px ${displayFont}`;ctx.fillText("MARIO BLANCO",540,990);
  roundedBox(ctx,718,688,294,380,24,"#071a3bef",gold,3);ctx.fillStyle=gold;ctx.font=`400 25px ${displayFont}`;ctx.fillText(copy.qrTitle,865,735);ctx.fillStyle=white;ctx.font=`400 34px ${displayFont}`;ctx.fillText(copy.qrAmount,865,780);if(english){ctx.fillStyle="#b9c7da";ctx.font=`700 18px ${bodyFont}`;ctx.fillText(copy.qrAccount,865,807);}ctx.fillStyle=white;const qrBoxY=english?818:803,qrImageY=english?828:813;roundedBox(ctx,765,qrBoxY,200,200,10,white,null,0);ctx.save();ctx.imageSmoothingEnabled=false;if(english){ctx.drawImage(paymentQr,35,310,602,602,775,qrImageY,180,180);}else{ctx.drawImage(paymentQr,775,qrImageY,180,180);}ctx.restore();ctx.fillStyle=gold;ctx.font=`800 15px ${bodyFont}`;ctx.fillText(copy.scan1,865,english?1039:1030);ctx.fillText(copy.scan2,865,english?1058:1052);
  roundedBox(ctx,68,1094,944,64,18,green,gold,2);ctx.fillStyle=white;canvasFontToFit(ctx,copy.confirmation,900,23,bodyFont,18);ctx.fillText(copy.confirmation,540,1135);
  roundedBox(ctx,68,1182,944,188,24,"#fff8e5f2",null,0);ctx.fillStyle=navy;ctx.font=`400 30px ${displayFont}`;ctx.fillText(copy.reminder,540,1228);ctx.fillStyle=green;ctx.font=`800 24px ${bodyFont}`;ctx.fillText(copy.deadline,540,1273);ctx.fillStyle=navy;ctx.font=`700 21px ${bodyFont}`;ctx.fillText(copy.numbers1,540,1316);ctx.font=`800 22px ${bodyFont}`;ctx.fillText(copy.numbers2,540,1350);
  roundedBox(ctx,68,1408,944,276,24,"#03112bef",gold,3);const polioW=150,polioH=polioW*(endPolio.height/endPolio.width);ctx.drawImage(endPolio,100,1442,polioW,polioH);ctx.fillStyle=red;ctx.fillRect(280,1438,3,182);ctx.textAlign="left";ctx.fillStyle=gold;ctx.font=`400 23px ${displayFont}`;ctx.fillText(copy.cause1,318,1476);ctx.fillText(copy.cause2,318,1509);ctx.fillStyle=white;ctx.font=`700 21px ${bodyFont}`;drawWrappedText(ctx,copy.causeBody,318,1551,375,30);const mottoW=255,mottoH=mottoW*(motto.height/motto.width),mottoX=727,mottoY=1546-mottoH/2;ctx.drawImage(motto,mottoX,mottoY,mottoW,mottoH);
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 17px ${displayFont}`;ctx.fillText(copy.teams,540,1725);teamLogos.forEach((logo,index)=>{const size=58,x=82+index*78,y=1742;ctx.fillStyle="#ffffffee";ctx.beginPath();ctx.arc(x+size/2,y+size/2,size/2+4,0,Math.PI*2);ctx.fill();ctx.drawImage(logo,x,y,size,size);});
  ctx.textAlign="center";ctx.fillStyle=gold;canvasFontToFit(ctx,copy.thanks,940,25,displayFont,20);ctx.fillText(copy.thanks,540,1854);
  const blob=await new Promise<Blob>((resolve,reject)=>canvas.toBlob((value)=>value?resolve(value):reject(new Error("No se pudo crear la imagen")),"image/jpeg",.95));return{blob,url:canvas.toDataURL("image/jpeg",.95)};
}

async function createReportFlyer(rows:ReportRow[]){
  const visibleRows=rows,rowHeight=Math.max(34,Math.min(56,Math.floor(820/Math.max(Math.min(visibleRows.length,26),1)))),extraHeight=Math.max(0,visibleRows.length-26)*rowHeight,flyerHeight=1920+extraHeight;
  ...
  const stadium=await loadCanvasImage("/flyer-stadium-bg.png");ctx.drawImage(stadium,0,0,1080,flyerHeight);
  const shade=ctx.createLinearGradient(0,0,0,flyerHeight);shade.addColorStop(0,"#020a1de8");shade.addColorStop(.48,"#061b3ed1");shade.addColorStop(.82,"#031128dc");shade.addColorStop(1,"#02091899");ctx.fillStyle=shade;ctx.fillRect(0,0,1080,flyerHeight);ctx.fillStyle=gold;ctx.fillRect(0,0,1080,14);ctx.fillRect(0,flyerHeight-14,1080,14);
  ...
  const totals={reserved:rows.reduce((n,row)=>n+row.reserved,0),paid:rows.reduce((n,row)=>n+row.paid,0),treasury:rows.reduce((n,row)=>n+row.treasury,0),total:rows.reduce((n,row)=>n+row.total,0)};
  roundedBox(ctx,72,458,936,112,20,"#071a3be8",gold,3);const metrics=[[String(totals.reserved),"RESERVADAS",gold],[String(totals.paid),"PAGADAS",green],[String(totals.treasury),"TESORERÍA",blue],[String(totals.total),"COLOCADAS",white]];metrics.forEach(([value,label,color],index)=>{const x=189+index*234;ctx.fillStyle=color;ctx.font=`400 42px ${displayFont}`;ctx.fillText(value,x,510);ctx.fillStyle="#b9c7da";ctx.font=`800 15px ${bodyFont}`;ctx.fillText(label,x,544);});
  ...
  const tableY=692,tableH=58+visibleRows.length*rowHeight+58;
  roundedBox(ctx,60,tableY-18,960,tableH+36,24,"#03112bec",gold,3);ctx.fillStyle="#0d2e61";ctx.fillRect(82,tableY,916,58);ctx.textBaseline="middle";ctx.fillStyle=gold;ctx.font=`400 17px ${displayFont}`;ctx.textAlign="center";ctx.fillText("#",110,tableY+29);ctx.textAlign="left";ctx.fillText("SOCIO",145,tableY+29);[["RESERVADAS",585],["PAGADAS",720],["TESORERÍA",855],["TOTAL",970]].forEach(([label,x])=>{ctx.textAlign="center";ctx.fillText(String(label),Number(x),tableY+29);});
  ...
  visibleRows.forEach((row,index)=>{const y=tableY+58+index*rowHeight;if(index%2===0){ctx.fillStyle="#ffffff0b";ctx.fillRect(82,y,916,rowHeight);}ctx.textBaseline="middle";ctx.fillStyle="#b9c7da";ctx.textAlign="center";ctx.font=`700 ${Math.max(18,Math.min(23,rowHeight*.43))}px ${bodyFont}`;ctx.fillText(String(index+1),110,y+rowHeight/2);ctx.fillStyle=white;ctx.textAlign="left";ctx.font=`700 ${Math.max(18,Math.min(24,rowHeight*.44))}px ${bodyFont}`;ctx.fillText(fitCanvasText(ctx,row.name,350),145,y+rowHeight/2);ctx.font=`400 ${Math.max(20,Math.min(26,rowHeight*.49))}px ${displayFont}`;[[row.reserved,585,gold],[row.paid,720,green],[row.treasury,855,blue],[row.total,970,white]].forEach(([value,x,color])=>{ctx.fillStyle=String(color);ctx.textAlign="center";ctx.fillText(String(value||"—"),Number(x),y+rowHeight/2);});});
  const totalY=tableY+58+visibleRows.length*rowHeight;ctx.fillStyle="#0d2e61";ctx.fillRect(82,totalY,916,58);ctx.fillStyle=gold;ctx.fillRect(82,totalY,916,3);ctx.font=`400 21px ${displayFont}`;ctx.textAlign="left";ctx.fillText("TOTAL DEL EQUIPO",145,totalY+29);[[totals.reserved,585,gold],[totals.paid,720,green],[totals.treasury,855,blue],[totals.total,970,white]].forEach(([value,x,color])=>{ctx.fillStyle=String(color);ctx.textAlign="center";ctx.fillText(String(value),Number(x),totalY+29);});ctx.strokeStyle="#ffffff24";ctx.lineWidth=1;[510,652,787,925].forEach((x)=>{ctx.beginPath();ctx.moveTo(x,tableY);ctx.lineTo(x,totalY+58);ctx.stroke();});ctx.textBaseline="alphabetic";
  const footerY=flyerHeight-73,tableBottom=totalY+76,messageY=tableBottom+(footerY-tableBottom)/2-21;ctx.fillStyle=white;ctx.textAlign="center";ctx.font=`400 34px ${displayFont}`;ctx.fillText("¡EL SIGUIENTE AVANCE LO CONSTRUIMOS JUNTOS!",540,messageY);ctx.fillStyle="#c2cee0";ctx.font=`600 22px ${bodyFont}`;ctx.fillText("Comparte la causa, invita a participar y coloca tu próxima casilla.",540,messageY+42);roundedBox(ctx,75,footerY,930,53,15,gold,null,0);ctx.fillStyle=navy;ctx.font=`400 25px ${displayFont}`;ctx.fillText("100 CASILLAS · 17 JUEGOS · UNA GRAN CAUSA",540,footerY+35);
  const blob=await new Promise<Blob>((resolve,reject)=>canvas.toBlob((value)=>value?resolve(value):reject(new Error("No se pudo crear la imagen")),"image/png"));return{blob,url:canvas.toDataURL("image/png")};
}
...
async function createFlyer(squares: Square[],language:"es"|"en"="es",season:SeasonConfig=defaultSeason,withSchedule=false) {
  ...
  const copy=english?{
    ...
    legend:["AVAILABLE","RESERVED","PAID","IN TREASURY"],rulesTitle:"CLEAR RULES, BIG FUN!",rules:[`$${season.squarePrice} USD per square. ${season.paymentDeadline?`Payment in full by ${formatPaymentDeadline(season.paymentDeadline,"en")}.`:"Payment deadline to be announced."}`,`$${season.gamePrize} USD for each participating game; ${season.games.length} chances to win.`,"Numbers are drawn at the start of the season and remain hidden until then.","Final score counts, including overtime; use the last digit for each team."],exampleTitle:"EXAMPLE · FINAL SCORE",exampleCopy:"Visitor 17 and Home 10: the winning square is where column 7 crosses row 0.",
    ...
    name:`QUINIELA MNF ${season.name}`,headline:"JUEGA. GANA. AYUDA.",intro:"Aparta una casilla y transforma cada lunes en una buena causa.",offer:`$${season.squarePrice} USD POR CASILLA   •   $${season.gamePrize} USD POR JUEGO   •   ${season.games.length} JUEGOS`,
    legend:["DISPONIBLE","RESERVADA","PAGADA","TESORERÍA"],rulesTitle:"¡REGLAS CLARAS, DIVERSIÓN GRANDE!",rules:[`$${season.squarePrice} USD por casilla. ${season.paymentDeadline?`Pago total antes del ${formatPaymentDeadline(season.paymentDeadline,"es")}.`:"Fecha límite de pago por definir."}`,`$${season.gamePrize} USD por cada juego participante; tienes ${season.games.length} oportunidades de ganar.`,"Los números se sortean al inicio de temporada y permanecen ocultos hasta entonces.","Cuenta el marcador final, incluidos tiempos extra, usando la unidad de cada equipo."],exampleTitle:"EJEMPLO · MARCADOR FINAL",exampleCopy:"Visitante 17 y Casa 10: gana la casilla donde la columna 7 cruza con el renglón 0.",
  ...
  const legend=[{label:copy.legend[0],color:cream,text:navy},{label:copy.legend[1],color:gold,text:navy},{label:copy.legend[2],color:green,text:white},{label:copy.legend[3],color:blue,text:white}];
  ctx.textAlign="left"; legend.forEach((item,index)=>{ctx.font=`400 18px ${displayFont}`;const sectionCenter=122+(index+.5)*(836/4),groupWidth=20+8+ctx.measureText(item.label).width,lx=sectionCenter-groupWidth/2;ctx.fillStyle=item.color;ctx.fillRect(lx,577+headerShift,20,20);ctx.fillStyle=white;ctx.fillText(item.label,lx+28,595+headerShift);});
...
function countWrappedTextLines(ctx:CanvasRenderingContext2D,text:string,maxWidth:number){const words=text.split(" ");let line="",lines=1;for(const word of words){const test=`${line}${word} `;if(line&&ctx.measureText(test).width>maxWidth){lines+=1;line=`${word} `;}else line=test;}return lines;}
...
function flyerFilename(type:FlyerType){return type==="winner-en"?"mnf-football-pool-2026-winner-en.png":type==="winner"?"ganador-quiniela-mnf-2026.png":type==="payment-en"?"mnf-football-pool-2026-payment.jpg":type==="payment"?"cobro-casillas-quiniela-mnf-2026.jpg":type==="report"?"avance-quiniela-mnf-2026.png":type==="schedule-en"?"mnf-football-pool-2026-games-en.png":type==="schedule"?"quiniela-mnf-2026-juegos.png":type==="board-en"?"mnf-football-pool-2026-en.png":"quiniela-mnf-2026.png";}
...
function labelFor(status:Status){ return status==="available"?"Disponible":status==="reserved"?"Reservada":status==="paid"?"Pagada":"Tesorería"; }
...
function roleHelp(role:Role,boardLocked=false){ if(role==="admin")return "Puedes modificar cualquier dato, administrar accesos y configurar los números.";if(boardLocked)return role==="treasury"?"El tablero está cerrado. Puedes confirmar pagos y registrar las casillas recibidas en Tesorería.":"El tablero está cerrado. Puedes consultar las casillas, pero ya no reservarlas ni editarlas.";return role==="treasury"?"Puedes reservar casillas, confirmar pagos y registrar las casillas recibidas en Tesorería.":"Puedes reservar casillas disponibles y actualizar los datos de las que tú vendiste. Tesorería confirmará los pagos."; }
...
function gameDateLabel(gameId:number,games:Game[],seasonName:string){const game=games[gameId-1];return game?`${game.date} ${seasonName}`:"";}
function formatPaymentDeadline(value:string,language:"es"|"en"){const [year,month,day]=value.split("-").map(Number);if(!year||!month||!day)return language==="es"?"fecha por definir":"date to be announced";return new Intl.DateTimeFormat(language==="es"?"es-MX":"en-US",{day:"numeric",month:"long",year:"numeric"}).format(new Date(year,month-1,day));}
