  ...
  return <main>
    ...
    {tab === "board" && <section className="content board-section">
      ...
      <div className="summary-grid">
        ...
        <button className={filter === "treasury" ? "summary treasury-summary active" : "summary treasury-summary"} onClick={() => setFilter(filter === "treasury" ? "all" : "treasury")}><span className="dot treasury"/><div><strong>{counts.treasury}</strong><small>En Tesorería</small></div></button>
      ...
      <div className="board-card"><div className="board-meta"><div className="legend"><span><i className="available"/>Disponible</span><span><i className="reserved"/>Reservada</span><span><i className="paid"/>Pagada</span><span><i className="treasury"/>En Tesorería</span></div><p>{boardLocked ? "Tablero cerrado · números de juego cargados" : "Toca una casilla para ver sus datos"}</p></div>
...
async function createReportFlyer(rows:ReportRow[]){
  ...
  const totals={reserved:rows.reduce((n,row)=>n+row.reserved,0),paid:rows.reduce((n,row)=>n+row.paid,0),treasury:rows.reduce((n,row)=>n+row.treasury,0),total:rows.reduce((n,row)=>n+row.total,0)};
  roundedBox(ctx,72,458,936,112,20,"#071a3be8",gold,3);const metrics=[[String(totals.reserved),"RESERVADAS",gold],[String(totals.paid),"PAGADAS",green],[String(totals.treasury),"EN TESORERÍA",blue],[String(totals.total),"COLOCADAS",white]];metrics.forEach(([value,label,color],index)=>{const x=189+index*234;ctx.fillStyle=color;ctx.font=`400 42px ${displayFont}`;ctx.fillText(value,x,510);ctx.fillStyle="#b9c7da";ctx.font=`800 15px ${bodyFont}`;ctx.fillText(label,x,544);});
...
function labelFor(status:Status){ return status==="available"?"Disponible":status==="reserved"?"Reservada":status==="paid"?"Pagada":"En Tesorería"; }
