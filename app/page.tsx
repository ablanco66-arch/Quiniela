...
async function createWinnerFlyer({result,game,square,visitorDigit,homeDigit}:WinnerRow){
  ...
  ctx.save();ctx.globalAlpha=.26;ctx.strokeStyle=white;ctx.lineWidth=3;for(let y=292;y<1710;y+=110){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(1080,y);ctx.stroke();for(let x=62;x<1020;x+=96){ctx.beginPath();ctx.moveTo(x,y-10);ctx.lineTo(x,y+10);ctx.stroke();}}ctx.font=`400 28px ${displayFont}`;ctx.fillStyle=white;ctx.textAlign="center";[20,30,40,50,40,30,20].forEach((yard,index)=>ctx.fillText(String(yard),95+index*148,1060));ctx.restore();
  ...
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 28px ${displayFont}`;ctx.fillText("QUINIELA MNF 2026",540,276);ctx.fillStyle=white;ctx.font=`400 78px ${displayFont}`;ctx.strokeStyle=navy;ctx.lineWidth=9;ctx.strokeText("¡TENEMOS GANADOR!",540,363);ctx.fillText("¡TENEMOS GANADOR!",540,363);ctx.fillStyle=cream;ctx.font=`400 25px ${displayFont}`;ctx.fillText("TU PARTICIPACIÓN HACE POSIBLE GRANDES CAUSAS",540,410);
  ...
  roundedBox(ctx,86,528,908,270,26,"#051631d9",gold,4);ctx.fillStyle=gold;ctx.font=`400 27px ${displayFont}`;ctx.fillText("FELICIDADES",540,606);ctx.fillStyle=white;ctx.font=`400 74px ${displayFont}`;ctx.strokeStyle=navy;ctx.lineWidth=11;const winnerName=fitCanvasText(ctx,(square.participant||"CASILLA GANADORA").toUpperCase(),800);ctx.strokeText(winnerName,540,704);ctx.fillText(winnerName,540,704);ctx.fillStyle=gold;ctx.fillRect(250,752,580,5);
