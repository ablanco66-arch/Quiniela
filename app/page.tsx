...
async function createPaymentFlyer(){
  ...
  roundedBox(ctx,68,986,944,188,24,"#fff8e5f2",null,0);ctx.fillStyle=navy;ctx.font=`400 30px ${displayFont}`;ctx.fillText("TU PAGO HACE LA DIFERENCIA",540,1036);ctx.font=`700 23px ${bodyFont}`;ctx.fillText("Gracias por participar, cumplir tu compromiso",540,1082);ctx.fillText("y ayudarnos a impulsar proyectos con fines altruistas.",540,1118);ctx.fillStyle=green;ctx.font=`800 20px ${bodyFont}`;ctx.fillText("¡CADA CASILLA SUMA!",540,1152);
  roundedBox(ctx,68,1212,944,276,24,"#03112bef",gold,3);const polioW=150,polioH=polioW*(endPolio.height/endPolio.width);ctx.drawImage(endPolio,105,1246,polioW,polioH);ctx.fillStyle=red;ctx.fillRect(286,1242,3,182);ctx.textAlign="left";ctx.fillStyle=gold;ctx.font=`400 23px ${displayFont}`;ctx.fillText("COOPERAMOS PARA HACER EL BIEN",324,1284);ctx.fillStyle=white;ctx.font=`700 21px ${bodyFont}`;drawWrappedText(ctx,"Tu participación fortalece el trabajo solidario de Rotary y la lucha para poner fin a la polio.",324,1330,620,30);const mottoW=330,mottoH=mottoW*(motto.height/motto.width);ctx.drawImage(motto,324,1400-mottoH/2,mottoW,mottoH);
