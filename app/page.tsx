...
async function createPaymentFlyer(){
  ...
  const [stadium,clubLogo,motto,endPolio]=await Promise.all([loadCanvasImage("/flyer-stadium-bg.png"),loadCanvasImage("/logo-crjc-white-gold.png"),loadCanvasImage("/lema-rotario-2026-2027.png"),loadCanvasImage("/end-polio-now.png")]);
  ...
  ctx.save();ctx.globalAlpha=.08;ctx.strokeStyle=gold;ctx.lineWidth=2;for(let y=40;y<1580;y+=92){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(1080,y);ctx.stroke();}ctx.restore();
  const clubW=490,clubH=clubW*(clubLogo.height/clubLogo.width);ctx.drawImage(clubLogo,(1080-clubW)/2,34,clubW,clubH);
