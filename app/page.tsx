...
async function createWinnerFlyer({result,game,square,visitorDigit,homeDigit,seasonName,gamePrize}:WinnerRow){
  ...
  ctx.save();const fieldTop=300,fieldBottom=1775,fieldLeft=42,fieldRight=1038;ctx.beginPath();ctx.rect(fieldLeft,fieldTop,fieldRight-fieldLeft,fieldBottom-fieldTop);ctx.clip();const turf=ctx.createLinearGradient(fieldLeft,0,fieldRight,0);turf.addColorStop(0,"#174f2d18");turf.addColorStop(.5,"#23824636");turf.addColorStop(1,"#174f2d18");ctx.fillStyle=turf;ctx.fillRect(fieldLeft,fieldTop,fieldRight-fieldLeft,fieldBottom-fieldTop);for(let x=fieldLeft;x<fieldRight;x+=108){ctx.fillStyle=((x-fieldLeft)/108)%2===0?"#43a85a0d":"#0c4d2816";ctx.fillRect(x,fieldTop,108,fieldBottom-fieldTop);}ctx.globalAlpha=.34;ctx.strokeStyle=white;ctx.fillStyle=white;ctx.lineCap="butt";ctx.textAlign="center";ctx.lineWidth=5;[fieldTop+10,fieldBottom-10].forEach((y)=>{ctx.beginPath();ctx.moveTo(fieldLeft,y);ctx.lineTo(fieldRight,y);ctx.stroke();});const yardLines=[80,260,540,820,1000];yardLines.forEach((x,index)=>{ctx.lineWidth=index===2?6:4;ctx.beginPath();ctx.moveTo(x,fieldTop);ctx.lineTo(x,fieldBottom);ctx.stroke();});for(let y=342;y<fieldBottom-24;y+=82){[118,186,420,486,594,660,894,962].forEach((x)=>{ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(x-10,y);ctx.lineTo(x+10,y);ctx.stroke();});}ctx.font=`400 42px ${sportFont}`;[[170,"30"],[350,"40"],[540,"50"],[730,"40"],[910,"30"]].forEach(([x,label])=>{ctx.fillText(String(label),Number(x),fieldTop+67);ctx.fillText(String(label),Number(x),fieldBottom-32);});ctx.restore();
