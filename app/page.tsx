...
async function createFlyer(squares: Square[],language:"es"|"en"="es",season:SeasonConfig=defaultSeason,withSchedule=false) {
  ...
  season.games.forEach((game,index)=>{const column=index<9?0:1,row=index<9?index:index-9,x=88+column*462,y=1540+row*56;if(row%2===0){ctx.fillStyle="#ffffff12";ctx.fillRect(x,y,442,51);}ctx.textBaseline="middle";ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 19px ${displayFont}`;ctx.fillText(String(index+1).padStart(2,"0"),x+28,y+25);ctx.fillStyle=white;ctx.font=`800 17px ${bodyFont}`;ctx.fillText(game.date.toUpperCase(),x+118,y+25);const visitorLogo=teamLogos.get(game.visitor),homeLogo=teamLogos.get(game.home);if(visitorLogo)ctx.drawImage(visitorLogo,x+232,y+1,49,49);if(homeLogo)ctx.drawImage(homeLogo,x+358,y+1,49,49);ctx.fillStyle=gold;ctx.font=`400 22px ${displayFont}`;ctx.fillText("@",x+320,y+25);});ctx.textBaseline="alphabetic";
