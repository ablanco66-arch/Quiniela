...
async function createPaymentFlyer(){
  ...
  const [stadium,clubLogo,motto,endPolio,mnfLogo,...teamLogos]=await Promise.all([loadCanvasImage("/flyer-stadium-bg.png"),loadCanvasImage("/logo-crjc-white-gold.png"),loadCanvasImage("/lema-rotario-2026-2027.png"),loadCanvasImage("/end-polio-now.png"),loadCanvasImage("/logo-monday-night-football.png"),...['den','kc','nyg','lar'].map((team)=>loadCanvasImage(`/nfl-logos/${team}.png`))]);
  ctx.drawImage(stadium,0,0,1080,1620);const shade=ctx.createLinearGradient(0,0,0,1620);shade.addColorStop(0,"#020a1df2");shade.addColorStop(.52,"#061b3ee8");shade.addColorStop(1,"#020918f7");ctx.fillStyle=shade;ctx.fillRect(0,0,1080,1620);ctx.fillStyle=gold;ctx.fillRect(0,0,1080,14);ctx.fillRect(0,1606,1080,14);
  ctx.save();ctx.globalAlpha=.14;ctx.strokeStyle=white;ctx.lineWidth=2;for(let y=540;y<1510;y+=108){ctx.beginPath();ctx.moveTo(24,y);ctx.lineTo(1056,y);ctx.stroke();for(let x=70;x<1040;x+=48){ctx.beginPath();ctx.moveTo(x,y-8);ctx.lineTo(x,y+8);ctx.stroke();}}ctx.setLineDash([18,14]);ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(540,526);ctx.lineTo(540,1510);ctx.stroke();ctx.setLineDash([]);ctx.font=`400 18px ${displayFont}`;ctx.textAlign="center";[10,20,30,40,50,40,30,20,10].forEach((yard,index)=>{const y=570+index*108;ctx.fillStyle=white;ctx.fillText(String(yard),42,y);ctx.fillText(String(yard),1038,y);});ctx.restore();
  ...
  roundedBox(ctx,68,688,454,262,24,"#071a3bef",gold,3);ctx.fillStyle=gold;ctx.font=`400 27px ${displayFont}`;ctx.fillText("EN EFECTIVO",295,740);ctx.fillStyle=white;ctx.font=`400 54px ${displayFont}`;ctx.fillText("$100 USD",295,822);ctx.fillStyle="#b9c7da";ctx.font=`800 18px ${bodyFont}`;ctx.fillText("O BIEN",295,862);ctx.fillStyle=white;ctx.font=`400 47px ${displayFont}`;ctx.fillText("$1,800 MXN",295,920);
  roundedBox(ctx,558,688,454,262,24,"#071a3bef",gold,3);ctx.fillStyle=gold;ctx.font=`400 27px ${displayFont}`;ctx.fillText("TRANSFERENCIA",785,740);ctx.fillStyle=white;ctx.font=`800 19px ${bodyFont}`;ctx.fillText("CLABE BBVA",785,790);ctx.font=`400 28px ${displayFont}`;ctx.fillText("012180015213044882",785,838);ctx.fillStyle="#b9c7da";ctx.font=`700 17px ${bodyFont}`;ctx.fillText("A NOMBRE DE",785,884);ctx.fillStyle=white;ctx.font=`400 27px ${displayFont}`;ctx.fillText("MARIO BLANCO",785,924);
  roundedBox(ctx,68,986,944,188,24,"#fff8e5f2",null,0);ctx.fillStyle=navy;ctx.font=`400 30px ${displayFont}`;ctx.fillText("¡QUE NO SE TE PASE!",540,1032);ctx.fillStyle=green;ctx.font=`800 24px ${bodyFont}`;ctx.fillText("FECHA LÍMITE DE PAGO: 13 DE SEPTIEMBRE",540,1077);ctx.fillStyle=navy;ctx.font=`700 21px ${bodyFont}`;ctx.fillText("Los números con los que jugarán las casillas se compartirán",540,1120);ctx.font=`800 22px ${bodyFont}`;ctx.fillText("el lunes 14 de septiembre.",540,1154);
  roundedBox(ctx,68,1212,944,276,24,"#03112bef",gold,3);const polioW=150,polioH=polioW*(endPolio.height/endPolio.width);ctx.drawImage(endPolio,100,1246,polioW,polioH);ctx.fillStyle=red;ctx.fillRect(280,1242,3,182);ctx.textAlign="left";ctx.fillStyle=gold;ctx.font=`400 21px ${displayFont}`;ctx.fillText("COOPERAMOS PARA",318,1280);ctx.fillText("HACER EL BIEN",318,1310);ctx.fillStyle=white;ctx.font=`700 19px ${bodyFont}`;drawWrappedText(ctx,"Tu participación fortalece el trabajo solidario de Rotary y la lucha para poner fin a la polio.",318,1350,375,28);const mottoW=255,mottoH=mottoW*(motto.height/motto.width),mottoX=727,mottoY=1350-mottoH/2;ctx.drawImage(motto,mottoX,mottoY,mottoW,mottoH);
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 25px ${displayFont}`;ctx.fillText("GRACIAS POR GENERAR UN IMPACTO DURADERO",540,1554);
