...
function Reports({ rows, squares, visitorDigits, homeDigits, gameResults, activity, me, members, saving, generatingFlyer, onGenerateFlyer, onGenerateWinnerFlyer, onSaveMember, onRemoveMember, onSaveGameResult, onClearGameResult, onClearAllGameResults }:{ rows:ReportRow[]; squares:Square[]; visitorDigits:string; homeDigits:string; gameResults:GameResult[]; activity:Activity[]; me:Member; members:Member[]; saving:boolean; generatingFlyer:boolean; onGenerateFlyer:()=>void; onGenerateWinnerFlyer:(winner:WinnerRow)=>Promise<void>; onSaveMember:(member:Member,originalEmail?:string)=>Promise<boolean>; onRemoveMember:(member:Member)=>Promise<boolean>; onSaveGameResult:(gameId:number,visitorScore:number,homeScore:number)=>Promise<boolean>; onClearGameResult:(gameId:number)=>Promise<boolean>; onClearAllGameResults:()=>Promise<boolean> }) {
  ...
  return <section className="content reports-section"><div className="section-heading"><div><p className="kicker">Control y seguimiento</p><h3>Reportes</h3></div><span className="year-pill">{squares.filter((s)=>s.status!=="available").length} vendidas</span></div>
    ...
    {reportView === "winners" && <div className="winners-view">
      {me.role==="admin"&&<form className="report-card result-entry" onSubmit={async(event)=>{event.preventDefault();const result={gameId:resultGameId,visitorScore:Number(visitorScore),homeScore:Number(homeScore)};if(await onSaveGameResult(result.gameId,result.visitorScore,result.homeScore)){const winner=winnerFromResult(result);if(winner)await onGenerateWinnerFlyer(winner);}}}>
        ...
        <label>Juego<select value={resultGameId} onChange={(event)=>setResultGameId(Number(event.target.value))}>{games.map((game,index)=><option key={index+1} value={index+1}>{String(index+1).padStart(2,"0")} · {gameDateLabel(index+1)} · {game.visitor} vs {game.home}</option>)}</select></label>
