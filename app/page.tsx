...
async function createPaymentFlyer(){
  ...
  const clubW=430,clubH=clubW*(clubLogo.height/clubLogo.width);ctx.drawImage(clubLogo,72,34,clubW,clubH);const mnfW=168,mnfH=mnfW*(mnfLogo.height/mnfLogo.width);ctx.drawImage(mnfLogo,840,48,mnfW,mnfH);teamLogos.forEach((logo,index)=>{const size=50,x=568+index*66,y=168;ctx.fillStyle="#ffffffee";ctx.beginPath();ctx.arc(x+size/2,y+size/2,size/2+5,0,Math.PI*2);ctx.fill();ctx.drawImage(logo,x,y,size,size);});
  ...
  roundedBox(ctx,68,1408,944,276,24,"#03112bef",gold,3);const polioW=150,polioH=polioW*(endPolio.height/endPolio.width);ctx.drawImage(endPolio,100,1442,polioW,polioH);ctx.fillStyle=red;ctx.fillRect(280,1438,3,182);ctx.textAlign="left";ctx.fillStyle=gold;ctx.font=`400 21px ${displayFont}`;ctx.fillText("COOPERAMOS PARA",318,1476);ctx.fillText("HACER EL BIEN",318,1506);ctx.fillStyle=white;ctx.font=`700 19px ${bodyFont}`;drawWrappedText(ctx,"Tu participación fortalece el trabajo solidario de Rotary y la lucha para poner fin a la polio.",318,1546,375,28);const mottoW=255,mottoH=mottoW*(motto.height/motto.width),mottoX=727,mottoY=1546-mottoH/2;ctx.drawImage(motto,mottoX,mottoY,mottoW,mottoH);
