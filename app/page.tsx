...
type FlyerType = "board" | "board-en" | "schedule" | "schedule-en" | "report" | "payment" | "winner";
type Square = {
...
export default function Home() {
  ...
  async function openPaymentFlyer() {
    setGeneratingFlyer(true);
    try {
      const result = await createPaymentFlyer();
      setFlyerType("payment"); setFlyerUrl(result.url); setFlyerBlob(result.blob); setShowFlyer(true);
  ...
  async function shareFlyer() {
    ...
    try {
      if (navigator.share && navigator.canShare?.({ files:[file] })) await navigator.share({ title:flyerType==="winner"?"¡Tenemos ganador MNF!":flyerType==="payment"?"Liquidación de casillas · Quiniela MNF 2026":flyerType==="board-en"?"MNF Football Pool 2026":"Quiniela MNF 2026", text:flyerType==="winner"?"¡Felicidades a nuestro ganador! Gracias por participar y apoyar causas que generan un impacto duradero.":flyerType==="payment"?"¡Gracias por sumarte a esta buena causa! Te compartimos las opciones para liquidar tus casillas pendientes de la Quiniela MNF 2026.":flyerType==="report"?"¡Vamos por las 100! Este es nuestro avance de ventas en la Quiniela MNF 2026.":flyerType==="board-en"?"Play for a great cause! Pick your square in the 2026 MNF Football Pool.":"¡Participa por una buena causa! Aparta tu casilla de la Quiniela MNF 2026.", files:[file] });
  ...
  return <main>
    ...
    {showFlyer && flyerUrl && <div className="modal-backdrop flyer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setShowFlyer(false)}><section className="flyer-modal" role="dialog" aria-modal="true" aria-labelledby="flyer-title"><button className="modal-close" onClick={() => setShowFlyer(false)} aria-label="Cerrar">×</button><div className="flyer-modal-head"><p className="kicker">{flyerType==="board-en"||flyerType==="schedule-en"?"Ready to share":"Listo para compartir"}</p><h3 id="flyer-title">{flyerType==="winner"?"Flier del ganador":flyerType==="payment"?"Flyer de cobro":flyerType==="report"?"Flier de avance":flyerType==="board-en"?"English flyer":flyerType==="schedule-en"?"English flyer + games":flyerType==="schedule"?"Flier con juegos MNF":"Flier de la quiniela"}</h3><p>{flyerType==="winner"?"Celebra y comparte al ganador de esta fecha.":flyerType==="payment"?"Comparte amablemente las opciones para liquidar las casillas pendientes.":flyerType==="report"?"El reporte refleja el avance más reciente por socio.":flyerType==="schedule-en"?"Board, rules, example and the full MNF schedule with team logos.":flyerType==="schedule"?"Tablero, reglas, ejemplo y calendario MNF completo con logos de los equipos.":flyerType==="board-en"?"The original English board flyer.":"El flier original con el estado actual del tablero."}</p></div><div className="flyer-preview"><img src={flyerUrl} alt={flyerType==="winner"?"Flier de felicitación al ganador de la Quiniela MNF":flyerType==="payment"?"Flyer con opciones para liquidar casillas pendientes":flyerType==="report"?"Flier del avance de casillas por socio":flyerType==="schedule-en"?"English MNF Football Pool flyer with board, rules and schedule":flyerType==="schedule"?"Flier vertical de la Quiniela MNF con tablero, reglas y calendario":flyerType==="board-en"?"English MNF Football Pool original flyer":"Flier original de la Quiniela MNF"}/></div><div className="flyer-actions"><button className="whatsapp-button" onClick={shareFlyer}>Compartir</button><button className="copy-button" onClick={copyFlyer}>Copiar imagen</button><button className="download-button" onClick={() => flyerBlob && downloadFlyer(flyerBlob,flyerType)}>Guardar {flyerType==="payment"?"JPG":"PNG"}</button></div></section></div>}
...
async function createPaymentFlyer(){
  const canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1920;
  const ctx=canvas.getContext("2d");if(!ctx)throw new Error("Canvas no disponible");
  ...
  const [stadium,clubLogo,motto,endPolio,mnfLogo,codiQr,...teamLogos]=await Promise.all([loadCanvasImage("/flyer-stadium-bg.png"),loadCanvasImage("/logo-crjc-white-gold.png"),loadCanvasImage("/lema-rotario-2026-2027.png"),loadCanvasImage("/end-polio-now.png"),loadCanvasImage("/logo-monday-night-football.png"),loadCanvasImage("/qr-codi-quiniela-mnf.jpeg"),...['den','kc','nyg','lar','phi','chi','atl','no','buf','wsh','sf','dal'].map((team)=>loadCanvasImage(`/nfl-logos/${team}.png`))]);
  ...
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 28px ${displayFont}`;ctx.fillText("QUINIELA MNF 2026",540,302);ctx.fillStyle=white;ctx.font=`400 67px ${displayFont}`;ctx.fillText("¡JUNTOS LLEGAMOS",540,382);ctx.fillText("A LA META!",540,450);ctx.fillStyle=cream;ctx.font=`700 27px ${bodyFont}`;ctx.fillText("Tu cooperación convierte cada casilla en apoyo para una buena causa.",540,500);
  roundedBox(ctx,68,542,944,112,24,gold,null,0);ctx.fillStyle=navy;ctx.font=`400 31px ${displayFont}`;ctx.fillText("AYÚDANOS A LIQUIDAR",540,588);ctx.font=`800 24px ${bodyFont}`;ctx.fillText("LAS CASILLAS QUE TENGAS PENDIENTES",540,626);
  roundedBox(ctx,68,688,294,380,24,"#071a3bef",gold,3);ctx.fillStyle=gold;ctx.font=`400 25px ${displayFont}`;ctx.fillText("EN EFECTIVO",215,740);ctx.fillStyle=white;ctx.font=`400 48px ${displayFont}`;ctx.fillText("$100 USD",215,835);ctx.fillStyle="#b9c7da";ctx.font=`800 18px ${bodyFont}`;ctx.fillText("O BIEN",215,882);ctx.fillStyle=white;ctx.font=`400 39px ${displayFont}`;ctx.fillText("$1,800 MXN",215,950);ctx.fillStyle=gold;ctx.font=`800 17px ${bodyFont}`;ctx.fillText("POR CASILLA",215,1002);
  roundedBox(ctx,393,688,294,380,24,"#071a3bef",gold,3);ctx.fillStyle=gold;ctx.font=`400 23px ${displayFont}`;ctx.fillText("TRANSFERENCIA",540,740);ctx.fillStyle=white;ctx.font=`400 35px ${displayFont}`;ctx.fillText("$1,800 MXN",540,795);ctx.font=`800 18px ${bodyFont}`;ctx.fillText("CLABE BBVA",540,848);ctx.font=`400 19px ${displayFont}`;ctx.fillText("012180015213044882",540,890);ctx.fillStyle="#b9c7da";ctx.font=`700 16px ${bodyFont}`;ctx.fillText("A NOMBRE DE",540,948);ctx.fillStyle=white;ctx.font=`400 24px ${displayFont}`;ctx.fillText("MARIO BLANCO",540,990);
  roundedBox(ctx,718,688,294,380,24,"#071a3bef",gold,3);ctx.fillStyle=gold;ctx.font=`400 25px ${displayFont}`;ctx.fillText("PAGA CON CoDi",865,735);ctx.fillStyle=white;ctx.font=`400 34px ${displayFont}`;ctx.fillText("$1,800 MXN",865,780);ctx.fillStyle=white;roundedBox(ctx,765,803,200,200,10,white,null,0);ctx.save();ctx.imageSmoothingEnabled=false;ctx.drawImage(codiQr,775,813,180,180);ctx.restore();ctx.fillStyle=gold;ctx.font=`800 15px ${bodyFont}`;ctx.fillText("ESCANEA EL QR CON",865,1030);ctx.fillText("TU APP BANCARIA",865,1052);
  roundedBox(ctx,68,1094,944,64,18,green,gold,2);ctx.fillStyle=white;ctx.font=`800 23px ${bodyFont}`;ctx.fillText("AL PAGAR, COMPARTE TU CONFIRMACIÓN DE PAGO",540,1135);
  roundedBox(ctx,68,1182,944,188,24,"#fff8e5f2",null,0);ctx.fillStyle=navy;ctx.font=`400 30px ${displayFont}`;ctx.fillText("¡QUE NO SE TE PASE!",540,1228);ctx.fillStyle=green;ctx.font=`800 24px ${bodyFont}`;ctx.fillText("FECHA LÍMITE DE PAGO: 13 DE SEPTIEMBRE",540,1273);ctx.fillStyle=navy;ctx.font=`700 21px ${bodyFont}`;ctx.fillText("Los números con los que jugarán las casillas se compartirán",540,1316);ctx.font=`800 22px ${bodyFont}`;ctx.fillText("el lunes 14 de septiembre.",540,1350);
  roundedBox(ctx,68,1408,944,276,24,"#03112bef",gold,3);const polioW=150,polioH=polioW*(endPolio.height/endPolio.width);ctx.drawImage(endPolio,100,1442,polioW,polioH);ctx.fillStyle=red;ctx.fillRect(280,1438,3,182);ctx.textAlign="left";ctx.fillStyle=gold;ctx.font=`400 23px ${displayFont}`;ctx.fillText("COOPERAMOS PARA",318,1476);ctx.fillText("HACER EL BIEN",318,1509);ctx.fillStyle=white;ctx.font=`700 21px ${bodyFont}`;drawWrappedText(ctx,"Tu participación fortalece el trabajo solidario de Rotary y la lucha para poner fin a la polio.",318,1551,375,30);const mottoW=255,mottoH=mottoW*(motto.height/motto.width),mottoX=727,mottoY=1546-mottoH/2;ctx.drawImage(motto,mottoX,mottoY,mottoW,mottoH);
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 17px ${displayFont}`;ctx.fillText("EQUIPOS MNF 2026",540,1725);teamLogos.forEach((logo,index)=>{const size=58,x=82+index*78,y=1742;ctx.fillStyle="#ffffffee";ctx.beginPath();ctx.arc(x+size/2,y+size/2,size/2+4,0,Math.PI*2);ctx.fill();ctx.drawImage(logo,x,y,size,size);});
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 25px ${displayFont}`;ctx.fillText("GRACIAS POR GENERAR UN IMPACTO DURADERO",540,1854);
