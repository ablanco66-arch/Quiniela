...
async function createWinnerFlyer({result,game,square,visitorDigit,homeDigit}:WinnerRow){
  ...
  ctx.drawImage(stadium,0,0,1080,1920);const shade=ctx.createLinearGradient(0,0,0,1920);shade.addColorStop(0,"#020817ec");shade.addColorStop(.45,"#061b3ed7");shade.addColorStop(1,"#020918f5");ctx.fillStyle=shade;ctx.fillRect(0,0,1080,1920);
  ctx.save();ctx.globalAlpha=.11;ctx.strokeStyle=white;ctx.lineWidth=2;for(let y=285;y<1710;y+=110){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(1080,y);ctx.stroke();}ctx.restore();ctx.fillStyle=gold;ctx.fillRect(0,0,1080,14);ctx.fillRect(0,1906,1080,14);
  const clubW=410,clubH=clubW*(clubLogo.height/clubLogo.width);ctx.drawImage(clubLogo,58,28,clubW,clubH);const mnfW=205,mnfH=mnfW*(mnfLogo.height/mnfLogo.width);ctx.drawImage(mnfLogo,817,25,mnfW,mnfH);
  ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 28px ${displayFont}`;ctx.fillText("QUINIELA MNF 2026",540,276);ctx.fillStyle=white;ctx.font=`400 78px ${displayFont}`;ctx.fillText("¡TENEMOS GANADOR!",540,363);ctx.fillStyle=cream;ctx.font=`700 25px ${bodyFont}`;ctx.fillText("TU PARTICIPACIÓN HACE POSIBLE GRANDES CAUSAS",540,410);
  roundedBox(ctx,350,438,380,54,27,gold,null,0);ctx.fillStyle=navy;ctx.font=`400 23px ${displayFont}`;ctx.fillText(gameDateLabel(result.gameId).toUpperCase(),540,474);
  roundedBox(ctx,86,528,908,270,26,"#051631ee",gold,4);ctx.fillStyle="#ffffff12";ctx.beginPath();ctx.arc(540,664,104,0,Math.PI*2);ctx.fill();ctx.strokeStyle=gold;ctx.lineWidth=4;ctx.stroke();ctx.fillStyle=gold;ctx.font=`400 24px ${displayFont}`;ctx.fillText("FELICIDADES",540,588);ctx.fillStyle=white;ctx.font=`400 52px ${displayFont}`;ctx.fillText(fitCanvasText(ctx,square.participant||"CASILLA GANADORA",780),540,670);ctx.fillStyle="#c7d2e4";ctx.font=`800 20px ${bodyFont}`;ctx.fillText(`JUGADOR · CASILLA #${square.id}`,540,726);ctx.fillStyle=gold;ctx.font=`400 20px ${displayFont}`;ctx.fillText(`${labelFor(square.status).toUpperCase()} · MARCADOR ${visitorDigit} × ${homeDigit}`,540,765);
  ctx.save();ctx.shadowColor="#f7b50066";ctx.shadowBlur=36;ctx.fillStyle=gold;ctx.beginPath();ctx.arc(540,940,119,0,Math.PI*2);ctx.fill();ctx.restore();ctx.fillStyle=navy;ctx.font=`400 27px ${displayFont}`;ctx.fillText("PREMIO",540,907);ctx.font=`400 67px ${displayFont}`;ctx.fillText("$300",540,971);ctx.font=`400 23px ${displayFont}`;ctx.fillText("USD",540,1006);
  ...
  const scoreTeams=[[game.visitor,result.visitorScore,"VISITANTE",290],[game.home,result.homeScore,"CASA",790]] as const;scoreTeams.forEach(([team,score,side,x])=>{ctx.fillStyle=gold;ctx.font=`400 18px ${displayFont}`;ctx.fillText(side,x,1181);ctx.fillStyle=white;ctx.font=`700 27px ${bodyFont}`;ctx.fillText(fitCanvasText(ctx,team,350),x,1225);ctx.fillStyle=gold;ctx.font=`400 70px ${displayFont}`;ctx.fillText(String(score),x,1300);});ctx.fillStyle="#8fa0b8";ctx.font=`400 26px ${displayFont}`;ctx.fillText("VS",540,1252);
  ...
  ctx.fillStyle="#aebed3";ctx.font=`800 18px ${bodyFont}`;ctx.fillText("CASILLA COLOCADA POR",540,1516);ctx.fillStyle=white;ctx.font=`400 35px ${displayFont}`;ctx.fillText(fitCanvasText(ctx,square.reservedByName||square.contact||"ROTARY JUÁREZ CONCORDIA",800),540,1562);
  ctx.fillStyle=gold;ctx.font=`400 31px ${displayFont}`;ctx.fillText("¡GRACIAS POR JUGAR, GANAR Y AYUDAR!",540,1642);ctx.fillStyle="#d5deeb";ctx.font=`600 22px ${bodyFont}`;ctx.fillText("Cada casilla fortalece proyectos que transforman vidas.",540,1682);
