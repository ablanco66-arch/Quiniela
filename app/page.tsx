...
async function createFlyer(squares: Square[],language:"es"|"en"="es",season:SeasonConfig=defaultSeason,withSchedule=false) {
  ...
  season.games.forEach((game,index)=>{const y=1540+index*59;if(index%2===0){ctx.fillStyle="#ffffff12";ctx.fillRect(88,y,904,54);}ctx.textBaseline="middle";ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 21px ${displayFont}`;ctx.fillText(String(index+1).padStart(2,"0"),125,y+27);ctx.fillStyle=white;ctx.font=`800 19px ${bodyFont}`;ctx.fillText(game.date.toUpperCase(),260,y+27);const visitorLogo=teamLogos.get(game.visitor),homeLogo=teamLogos.get(game.home);if(visitorLogo)ctx.drawImage(visitorLogo,442,y+1,52,52);if(homeLogo)ctx.drawImage(homeLogo,586,y+1,52,52);ctx.fillStyle=gold;ctx.font=`400 24px ${displayFont}`;ctx.fillText("@",540,y+27);});ctx.textBaseline="alphabetic";
  ...
  rules.forEach(([number,text],index)=>{const y=2652+index*42;ctx.font=`700 18px ${bodyFont}`;const lineCount=countWrappedTextLines(ctx,text,535),circleY=y-7+((lineCount-1)*20)/2;ctx.fillStyle=gold;ctx.beginPath();ctx.arc(100,circleY,17,0,Math.PI*2);ctx.fill();ctx.fillStyle=navy;ctx.textAlign="center";ctx.font=`400 18px ${displayFont}`;ctx.fillText(number,100,circleY+6);ctx.fillStyle=white;ctx.textAlign="left";ctx.font=`700 18px ${bodyFont}`;drawWrappedText(ctx,text,130,y,535,20);});const motto=english?mottoEn:mottoEs,mottoW=250,mottoH=mottoW*(motto.height/motto.width),mottoY=2641+(150-mottoH)/2;ctx.fillStyle=gold;ctx.fillRect(700,2635,3,158);roundedBox(ctx,725,2635,285,158,18,"#051631a8",null,0);ctx.drawImage(motto,742,mottoY,mottoW,mottoH);
  roundedBox(ctx,70,2810,940,126,20,"#071a3be8",gold,2);ctx.textAlign="center";ctx.fillStyle=gold;ctx.font=`400 21px ${displayFont}`;ctx.fillText(copy.exampleTitle,540,2838);ctx.fillStyle="#d7e1ee";ctx.font=`800 15px ${bodyFont}`;ctx.fillText(english?"VISITOR":"VISITANTE",220,2872);ctx.fillText(english?"HOME":"CASA",860,2872);ctx.fillStyle=white;ctx.font=`400 42px ${displayFont}`;ctx.fillText("17",220,2907);ctx.fillText("10",860,2907);roundedBox(ctx,420,2856,240,55,12,gold,null,0);ctx.fillStyle=navy;ctx.font=`400 32px ${displayFont}`;ctx.fillText("7 × 0",540,2894);ctx.fillStyle=white;canvasFontToFit(ctx,copy.exampleCopy,900,18,bodyFont,15);ctx.fillText(copy.exampleCopy,540,2926);
  ctx.fillStyle="#e3e9f2";ctx.textAlign="center";canvasFontToFit(ctx,copy.warning,940,18,bodyFont,14);ctx.fillText(copy.warning,540,2980);
  roundedBox(ctx,70,3020,940,126,20,"#071a3be8",gold,3);ctx.fillStyle=white;canvasFontToFit(ctx,copy.cta,880,30,displayFont,22);ctx.fillText(copy.cta,540,3062);ctx.fillStyle=gold;ctx.font=`800 21px ${bodyFont}`;ctx.fillText(english?"GET YOUR SQUARE TODAY":"APARTA TU CASILLA HOY",540,3105);
