...
export default function Home() {
  ...
  return <main>
    ...
    {tab === "games" && <Games season={season} />}
    {tab === "rules" && <Rules season={season} />}
    {tab === "reports" && <Reports rows={report} squares={squares} visitorDigits={visitorDigits} homeDigits={homeDigits} gameResults={gameResults} activity={activity} me={me!} members={members} season={season} seasonArchives={seasonArchives} saving={saving} generatingFlyer={generatingFlyer} onGenerateFlyer={openReportFlyer} onGenerateWinnerFlyer={openWinnerFlyer} onSaveMember={saveMember} onRemoveMember={removeMember} onSaveGameResult={saveGameResult} onClearGameResult={clearGameResult} onClearAllGameResults={clearAllGameResults} onSaveSeason={saveSeason} />}
...
function Reports({ rows, squares, visitorDigits, homeDigits, gameResults, activity, me, members, season, seasonArchives, saving, generatingFlyer, onGenerateFlyer, onGenerateWinnerFlyer, onSaveMember, onRemoveMember, onSaveGameResult, onClearGameResult, onClearAllGameResults, onSaveSeason }:{ rows:ReportRow[]; squares:Square[]; visitorDigits:string; homeDigits:string; gameResults:GameResult[]; activity:Activity[]; me:Member; members:Member[]; season:SeasonConfig; seasonArchives:SeasonArchive[]; saving:boolean; generatingFlyer:boolean; onGenerateFlyer:()=>void; onGenerateWinnerFlyer:(winner:WinnerRow,language?:"es"|"en")=>Promise<void>; onSaveMember:(member:Member,originalEmail?:string)=>Promise<boolean>; onRemoveMember:(member:Member)=>Promise<boolean>; onSaveGameResult:(gameId:number,visitorScore:number,homeScore:number)=>Promise<boolean>; onClearGameResult:(gameId:number)=>Promise<boolean>; onClearAllGameResults:()=>Promise<boolean>; onSaveSeason:(season:SeasonConfig,activate?:boolean)=>Promise<boolean> }) {
  const [reportView,setReportView] = useState<"summary"|"squares"|"winners"|"activity"|"access"|"season">("summary");
  ...
  return <section className="content reports-section"><div className="section-heading"><div><p className="kicker">Control y seguimiento</p><h3>Reportes</h3></div><span className="year-pill">{squares.filter((s)=>s.status!=="available").length} vendidas</span></div>
    ...
    {reportView === "winners" && <div className="winners-view">
...
function Games({season}:{season:SeasonConfig}){ return <section className="content games-section"><div className="section-heading"><div><p className="kicker">Calendario oficial</p><h3>{season.games.length} lunes de emoción</h3></div><span className="year-pill">{season.name}</span></div><div className="games-list">{season.games.map((game,index)=><article className="game" key={`${game.date}-${game.visitor}-${index}`}><div className="game-number"><span>JUEGO</span><strong>{String(index+1).padStart(2,"0")}</strong></div><div className="game-date">{game.date}</div><div className="matchup"><div><small>VISITANTE</small><strong>{game.visitor}</strong></div><span>@</span><div><small>CASA</small><strong>{game.home}</strong></div></div><div className="monday">MNF</div></article>)}</div></section>; }
